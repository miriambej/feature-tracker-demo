...
function DeliveryPlan({ features, allocations, setAllocations, capacities, setCapacities, owners }) {
  ...
  const kanbanBase=planningFeatures.filter(f=>(planWorkspaceFilter==='ALL'||f.workspace===planWorkspaceFilter)&&(ownerFilter==='ALL'||splitOwners(f.owner).includes(ownerFilter)||entityAllocations(f).some(a=>a.owner===ownerFilter))&&entityAllocations(f).some(a=>a.sprint===sprint));
  const kanbanItems=kanbanBase.map(feature=>{const through=entityThrough(feature); const next=entityNext(feature); const sprintAllocations=entityAllocations(feature).filter(a=>a.sprint===sprint); return{feature,through,next,latest:entityLatestSprint(feature),sprintStage:sprintAllocations.map(a=>`${a.stage}${a.isStageComplete?' ✓':''}`).join(', ')||'—'};}).sort((a,b)=>priorityRank(a.feature.user_count)-priorityRank(b.feature.user_count)||Number(b.feature.user_count||0)-Number(a.feature.user_count||0));
  ...
  const kanbanPanel=<div className="panel planning-kanban-panel"><div className="panel-top"><div><h3>Planning Kanban</h3><p className="muted">Start here: highest-priority cards show the next planning decision from actual status and completed allocations.</p></div><div className="mini-stats"><span>{sprint}</span><span>{ownerFilter==='ALL'?'All Owners':ownerFilter}</span><span>{planWorkspaceFilter==='ALL'?'All Workspaces':planWorkspaceFilter}</span></div></div><div className="board-scroll"><div className="board compact planning-board">{kanbanColumns.map(col=><div className="board-col compact" key={col.stage}><div className="col-head"><div><b>{col.label}</b><small>{col.items.length} shown</small></div><span>{col.items.length}</span></div>{col.items.length?col.items.map(({feature,through,next,latest,sprintStage})=><div className={`feature-card compact ${priorityClass(feature.user_count)}`} key={feature.id} onClick={()=>openPlanFeature(feature)}><div className="card-top"><div className="card-title">{feature.feature_name}</div><button onClick={e=>{e.stopPropagation();openPlanFeature(feature);}}>Edit</button></div><div className="card-workspace">{feature.workspace}</div><div className="card-meta"><span>Current: {STATUS_LABEL[feature.status]||'Standalone'}</span><span>{Number(feature.user_count||0).toLocaleString()} users · {priority(feature.user_count)}</span></div><div className="card-meta"><span>Through: {through}</span><span>Next: {next}</span></div><div className="card-meta"><span>Latest: {latest}</span><span>{`Sprint: ${sprintStage}`}</span></div></div>):<div className="empty">Nothing here yet.</div>}</div>)}</div></div></div>;
  const allocationEditor=<div className="panel"><div className="panel-top"><div><h3>Planning Allocations</h3><p className="muted">Showing allocations for {sprint} · {ownerFilter==='ALL'?'All Owners':ownerFilter} · {planWorkspaceFilter==='ALL'?'All Workspaces':planWorkspaceFilter}</p></div><div className="toolbar-left"><select value={stageFilter} onChange={e=>setStageFilter(e.target.value)}><option value="ALL">All Stages</option>{PLAN_STAGES.map(s=><option key={s}>{s}</option>)}</select></div></div><div className="summary-grid allocation-summary"><SummaryCard label="Planned Days" value={filteredAllocations.reduce((n,a)=>n+Number(a.days||0),0)} hint="Current view"/><SummaryCard label="Over Capacity" value={capacityRows.filter(r=>r.remaining!=null&&r.remaining<0).length} hint="Owners in sprint"/><SummaryCard label="Allocations" value={filteredAllocations.length} hint="Current view"/></div><table><thead><tr><th>Sprint</th><th>Feature / Planning Item</th><th>Workspace</th><th>Stage</th><th>Owner</th><th>Days</th><th>Stage Outcome</th><th>Actions</th></tr></thead><tbody>{filteredAllocations.map(a=><tr key={a.id}><td><select value={a.sprint} onChange={e=>updateAllocation(a.id,{sprint:e.target.value})}>{sprintOptions.map(s=><option key={s}>{s}</option>)}</select></td><td><b>{a.planningGroup||a.featureName||a.workspaceName}</b>{a.planningGroup&&a.featureName&&<small>Child: {a.featureName}</small>}{isSplitPlanningGroup(a.planningGroup||a.featureName)&&<select value={a.featureId||''} onChange={e=>updateAllocationFeature(a.id,e.target.value)}><option value="">Select child feature</option>{getChildFeatureOptions(a,features).map(f=><option key={f.id} value={f.id}>{f.feature_name}</option>)}</select>}</td><td>{allocationWorkspace(a)}</td><td><select value={a.stage} onChange={e=>updateAllocation(a.id,{stage:e.target.value})}>{PLAN_STAGES.map(s=><option key={s}>{s}</option>)}</select></td><td><select value={a.owner} onChange={e=>updateAllocation(a.id,{owner:e.target.value})}>{ownerOptions.map(o=><option key={o}>{o}</option>)}</select><small>{remainingFor(a.owner,a.sprint)==null?'No capacity':remainingFor(a.owner,a.sprint)<0?`Over capacity by ${Math.abs(remainingFor(a.owner,a.sprint))} days`:`Remaining in ${a.sprint}: ${remainingFor(a.owner,a.sprint)} days`}</small></td><td><input type="number" min="0" value={a.days} onChange={e=>updateAllocation(a.id,{days:Number(e.target.value||0)})}/></td><td><select value={a.isStageComplete?'complete':'planned'} onChange={e=>updateAllocation(a.id,{isStageComplete:e.target.value==='complete'})}><option value="planned">Planned only</option><option value="complete">Planned and expected to complete</option></select></td><td><button onClick={()=>deleteAllocation(a.id)}>Delete</button></td></tr>)}</tbody></table>{!filteredAllocations.length&&<p className="muted">No allocations match the current filters.</p>}</div>;

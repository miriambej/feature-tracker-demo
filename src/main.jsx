...
function DeliveryPlan({ features, allocations, setAllocations, capacities, setCapacities, customSprints, setCustomSprints, owners }) {
  ...
  const capacityRows=visibleOwners.map(owner=>{const ownerKey=normalisePersonName(owner);const key=ownerSprintKey(ownerKey,sprint);const cap=capacityByOwnerSprint.get(key);const available=cap?Number(cap.availableDays||0):null;const planned=plannedByOwnerSprint.get(key)||0;return{owner:ownerKey,available,planned,remaining:available==null?null:available-planned,counted:[]};});
  function remainingFor(owner,sprintId){const key=ownerSprintKey(owner,sprintId);const cap=capacityByOwnerSprint.get(key); if(!cap)return null; return Number(cap.availableDays||0)-(plannedByOwnerSprint.get(key)||0);}
  ...
  async function importPlanningExcel(e){
    ...
    try{
      ...
      setImportMessage(`Imported ${imported.length} allocation(s). Imported ${importedCaps.length} capacity row(s).${styleWarning}${commentMessage}`);
  ...
  );  return <div className="dashboard"><div className="dash-head"><div><div className="eyebrow">Delivery Plan</div><h1>Stage-by-stage sprint planning</h1></div><div className="toolbar-left"><button onClick={()=>document.getElementById('planning-xlsx').click()}>Import 26Q1 Planning Excel</button><input id="planning-xlsx" type="file" accept=".xlsx,.xls" hidden onChange={importPlanningExcel}/><button onClick={exportPlanningData}>Export Planning Data</button><button onClick={()=>document.getElementById('planning-json').click()}>Import Planning Data</button><input id="planning-json" type="file" accept=".json,application/json" hidden onChange={importPlanningData}/><select value={ownerFilter} onChange={e=>setOwnerFilter(e.target.value)}><option value="ALL">All Owners</option>{ownerOptions.map(o=><option key={o}>{o}</option>)}</select><select value={planWorkspaceFilter} onChange={e=>setPlanWorkspaceFilter(e.target.value)}><option value="ALL">All Workspaces</option>{allFeatureWorkspaces.map(w=><option key={w}>{w}</option>)}</select></div></div><div className="panel planning-controls"><div className="toolbar-left"><input value={newSprint} onChange={e=>setNewSprint(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')addSprint();}} placeholder="Add sprint, e.g. 26Q2S1"/><button onClick={addSprint}>Add Sprint</button></div></div>{importMessage&&<div className="panel import-message">{importMessage}</div>}{importDiagnosticsPanel}{kanbanPanel}<div className="planning-detail-grid"><div className="panel sprint-planner-panel"><div className="panel-top compact-panel-top"><div><h3>Sprint Planner</h3><p className="muted">Capacity input for the selected sprint.</p></div><label>Capacity Sprint<select value={sprint} onChange={e=>setSprint(e.target.value)}>{sprintOptions.map(s=><option key={s}>{s}</option>)}</select></label></div><table className="compact-table capacity-table"><thead><tr><th>Owner</th><th>Avail</th><th>Plan</th><th>Remain</th></tr></thead><tbody>{capacityRows.map(r=><tr key={r.owner}><td>{r.owner}</td><td><input type="number" value={r.available??''} placeholder="—" onChange={e=>updateCapacity(r.owner,e.target.value)}/></td><td>{r.planned}</td><td className={r.remaining<0?'capacity-negative':''}>{r.remaining==null?'—':r.remaining}</td></tr>)}</tbody></table></div>{planBySprint}</div>{allocationMatrixEditor}{allocationEditModal}{planModal}</div>;
}
...
function App(){
 ...
 useEffect(()=>{if(!loaded)return;const handle=setTimeout(()=>localStorage.setItem(STORAGE_KEY,JSON.stringify({features,milestones,allocations,capacities,customSprints})),200);return()=>clearTimeout(handle);},[features,milestones,allocations,capacities,customSprints,loaded]);

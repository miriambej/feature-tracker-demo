...
function getChildFeatureOptions(allocation, features) {
  if (allocation.possibleMatches?.length) {
    const matched=features.filter(f=>allocation.possibleMatches.includes(f.feature_name));
...
function DeliveryPlan({ features, allocations, setAllocations, capacities, setCapacities, customSprints, setCustomSprints, owners }) {
  ...
  async function importPlanningExcel(e){
    ...
    try{
      ...
      const xmlCommentMap=(()=>{try{const comments={};Object.keys(wb.files||{}).forEach(name=>{const rawName=String(name);const cleanName=rawName.startsWith('/')?rawName.slice(1):rawName;if(cleanName.startsWith('xl/comments')&&cleanName.endsWith('.xml')){const xml=parseXml(fileText(cleanName));xml?.querySelectorAll('comment').forEach(node=>{const ref=node.getAttribute('ref');const txt=cleanCommentText(Array.from(node.querySelectorAll('t')).map(t=>t.textContent||'').join(' '));if(ref&&txt)comments[ref]=comments[ref]?[comments[ref],txt].join(' | '):txt;});}if(cleanName.startsWith('xl/threadedComments/threadedComment')&&cleanName.endsWith('.xml')){const xml=parseXml(fileText(cleanName));xml?.querySelectorAll('threadedComment').forEach(node=>{const ref=node.getAttribute('ref');const txt=cleanCommentText(node.querySelector('text')?.textContent||'');if(ref&&txt)comments[ref]=comments[ref]?[comments[ref],txt].join(' | '):txt;});}});return comments;}catch(err){console.warn('Planning Excel comment diagnostics failed',err);return{};}})();
      const stageComment=(r,c)=>{const ref=XLSX.utils.encode_cell({r,c});const fromXml=xmlCommentMap[ref]||'';const fromSheet=(rawCell(r,c)?.c||[]).map(c=>c.t||c.a||'').join(' | ');return cleanCommentText(fromXml||fromSheet);};
      ...
      for(let r=12;r<=range.e.r;r++){
        ...
        sprintCols.forEach(({sprint,col})=>{
          ...
          if(owner&&!/^total$/i.test(owner)&&days&&!isRolePlaceholder(owner)){
            ...
            diagnostics.push({row:r+1,stageCell:XLSX.utils.encode_cell({r,c:col+1}),sprint,person:owner,feature:currentFeature,stage:sourceStage||stage,days,fillColor:allocationFillColors,expectedComplete:isStageComplete,comment,matchedFeature:match?.feature_name||'',possibleMatches:possibleMatches.join(' | '),mappingStatus});
            imported.push({id:id(),featureId:match?.id||'',featureName:match?.feature_name||(split?'':currentFeature),actualFeatureName:match?.feature_name||'',planningGroup:split?currentFeature:'',workspaceName:currentFeature,workspace:match?.workspace||(!split?currentFeature:''),sprint,owner,stage,sourceStage,stageComment:comment,possibleMatches,mappingStatus,days,isStageComplete});
          }
        });
      }
  ...
  const importDiagnosticsPanel=importDiagnostics.length?<div className="panel import-diagnostics"><div className="panel-top"><div><h3>Planning Import Diagnostics</h3><p className="muted">Showing what the Excel importer read for allocation fills and comments.</p></div><span className="pill-status neutral">{importDiagnostics.length} rows</span></div><table className="compact-table"><thead><tr><th>Row</th><th>Cell</th><th>Sprint</th><th>Person</th><th>Feature</th><th>Stage</th><th>Days</th><th>Fill</th><th>Expected</th><th>Comment</th><th>Matched Feature</th></tr></thead><tbody>{importDiagnostics.slice(0,200).map((row,idx)=><tr key={`${row.row}-${row.sprint}-${idx}`}><td>{row.row}</td><td>{row.stageCell}</td><td>{row.sprint}</td><td>{row.person}</td><td>{row.feature}</td><td>{row.stage}</td><td>{row.days}</td><td>{row.fillColor}</td><td>{row.expectedComplete?'true':'false'}</td><td>{row.comment||'—'}</td><td>{row.matchedFeature||row.mappingStatus}{row.possibleMatches?` (${row.possibleMatches})`:''}</td></tr>)}</tbody></table>{importDiagnostics.length>200&&<p className="muted">Showing first 200 diagnostics rows.</p>}</div>:null;

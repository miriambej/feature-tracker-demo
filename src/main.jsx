...
function isAtLeast(status, target) {
  return score(status) >= score(target);
}
...
function priorityClass(count) {
  return priority(count).toLowerCase();
}
function nextStageLabel(status) {
  return READY_NEXT_STAGE[status] || "";
}
...
function allocationFeatureIds(allocation) {
  const ids = Array.isArray(allocation.matchedFeatureIds)
...
function allocationFeatureComplete(allocation, featureId) {
  if (
    featureId &&
    allocation.childOutcomes &&
    Object.prototype.hasOwnProperty.call(allocation.childOutcomes, featureId)
  )
    return !!allocation.childOutcomes[featureId];
  const ids = allocationFeatureIds(allocation);
  if (featureId && ids.length > 1)
    return featureId === allocation.featureId
...
function prodSupportStoryFeature(story) {
  return {
    id: prodSupportStoryId(story.id),
...
function commentStoryParts(comment) {
  ...
  text = text
    ...
    .replace(/\breply\s*:\s*/gi, " ");
  const devOpsId = text.match(/user\s*story\s*(\d+)/i)?.[1] || urlId;
  const ticketTitle = text.match(/\b[A-Z]{2,}\d+\s*-\s*(.+)$/i)?.[1] || "";
  const name = (ticketTitle || text)
    .replace(/user\s*story\s*\d+/gi, "")
    .replace(/\b[A-Z]{2,}\d+\s*-\s*/gi, "")
    .replace(/\bgoal\b/gi, "")
    .replace(/\s+/g, " ")
    .trim();
...
function actualPlannedThrough(status) {
  ...
  if (["deployment_in_progress", "deployment_done"].includes(status))
    return "Deploy";
  if (["bs_signoff_in_progress", "bs_signoff_done"].includes(status))
    return "BA Sign Off";
...
function boardPlanningFloor(status) {
  ...
  if (status === "sit_done" || status === "deployment_in_progress")
    return "Deploy";
  if (status === "deployment_done" || status === "bs_signoff_in_progress")
    return "BA Sign Off";
  if (status === "bs_signoff_done" || status === "uat_in_progress")
    return "UAT";
...
function effectivePlannedThrough(feature, allocations) {
  ...
  const plannedIdx = PLAN_STAGES.indexOf(
    plannedThrough(feature.id, allocations),
  );
  const max = Math.max(actualIdx, plannedIdx);
  return max >= 0 ? PLAN_STAGES[max] : "Not planned";
}
function nextPlanStage(featureId, allocations, feature = null) {
  const through = feature
    ? effectivePlannedThrough(feature, allocations)
    : plannedThrough(featureId, allocations);
  const idx = PLAN_STAGES.indexOf(through);
  return idx >= PLAN_STAGES.length - 1
    ? "Planning Complete"
    : PLAN_STAGES[idx + 1];
}
function planningBadge(featureId, allocations) {
  const next = nextPlanStage(featureId, allocations);
  return next === "Planning Complete"
    ? `Planned to ${PLAN_STAGES.at(-1)}`
    : plannedThrough(featureId, allocations) === "Not planned"
      ? ""
      : `Next: ${next}`;
}
function priorityRank(count) {
  return { Critical: 0, High: 1, Medium: 2, Low: 3 }[priority(count)] ?? 4;
}
function latestPlannedSprint(featureId, allocations) {
  const sprints = allocations
    .filter((a) => allocationHasFeature(a, featureId) && a.sprint)
    .map((a) => a.sprint)
    .sort();
  return sprints.at(-1) || "-";
}
...
function isTsnswAdultCommunityEducation(feature) {
  return /tsnsw\s+adult\s+community\s+education/i.test(
...
function defaultFinalStageForFeature(feature) {
  ...
  if (feature && !feature.planningKey && !feature.isProdSupportStory)
    return "BA Sign Off";
  return DEFAULT_FINAL_STAGE;
}
function nextPlanningStageFromRows(
  feature,
  rows,
  finalStage = DEFAULT_FINAL_STAGE,
) {
  ...
  const max = Math.max(
    actualIdx,
    plannedIdx.length ? Math.max(...plannedIdx) : -1,
  );
...
function normaliseWorkspaceName(v) {
  const name = String(v || "")
    .trim()
    .replace(/\s+/g, " ");
  return name.replace(/^PBI Dashboard Reporting\s+/i, "").trim();
}
function allocationSourceId(allocation) {
  return allocation.sourceAllocationId || allocation.id;
}
function expandSharedAllocation(allocation) {
  const featureIds = allocationFeatureIds(allocation);
  if (featureIds.length <= 1)
    return [
      {
        ...allocation,
        sourceAllocationId: allocationSourceId(allocation),
        workspace: normaliseWorkspaceName(allocation.workspace),
        workspaceName: normaliseWorkspaceName(allocation.workspaceName),
        planningGroup: normaliseWorkspaceName(allocation.planningGroup),
      },
    ];
  return featureIds.map((featureId, index) => ({
    ...allocation,
    id: index === 0 ? allocation.id : id(),
    sourceAllocationId: allocationSourceId(allocation),
    featureId,
    featureIds: [featureId],
    matchedFeatureIds: [featureId],
    days: index === 0 ? Number(allocation.days || 0) : 0,
    sharedVisibility: true,
    isStageComplete: allocationFeatureComplete(allocation, featureId),
    childOutcomes: {
      [featureId]: allocationFeatureComplete(allocation, featureId),
    },
    workspace: normaliseWorkspaceName(allocation.workspace),
    workspaceName: normaliseWorkspaceName(allocation.workspaceName),
    planningGroup: normaliseWorkspaceName(allocation.planningGroup),
...
function isRolePlaceholder(name) {
  return /^(developers?|testers?|tester|ba|bi|sd|solution designer|business analyst|developer)$/i.test(
    String(name || "").trim(),
  );
}
function isSplitPlanningGroup(name) {
  return (
    isProdSupportGroup(name) ||
    /(pbi|power\s*bi).*dashboards?|dashboard.*reporting|migration|workspace|parent/i.test(
      String(name || ""),
    )
  );
}
function planningKeywords(name) {
  return String(name || "")
    .toLowerCase()
    .replace(/power\s*bi|pbi|pba|dashboards?|reporting|[-&]/g, " ")
    .replace(/[^a-z0-9]+/g, " ")
    .split(/\s+/)
    .filter((w) => w && !["and", "the", "for", "of"].includes(w));
}
function getChildFeatureOptions(allocation, features) {
  const possibleNames = allocation.possibleMatches || [];
  const suggested = possibleNames.length
    ? features.filter((f) => possibleNames.includes(f.feature_name))
    : [];
  const label =
  ...
  return Array.from(byId.values()).sort((a, b) => {
    ...
    return (
      aSuggested - bSuggested || a.feature_name.localeCompare(b.feature_name)
    );
  });
}
...
function findFeatureFromComment(comment, parentLabel, features) {
  ...
  const scoped = features.filter((f) =>
    parentTokens.length
      ? parentTokens.some(
          (w) =>
            matchTokens(f.workspace).includes(w) ||
            matchTokens(f.feature_name).includes(w),
        )
      : true,
  );
  ...
  const contained = pool
    .map((f) => {
      const key = matchKey(f.feature_name);
      return { feature: f, index: key ? commentKey.indexOf(key) : -1 };
    })
    .filter((x) => x.index >= 0)
    .sort(
      (a, b) =>
  ...
  const candidates = pool
    .map((f) => {
      const featureTokens = Array.from(
        new Set([...matchTokens(f.feature_name), ...matchTokens(f.workspace)]),
      );
      const hits = featureTokens.filter((t) => commentTokens.includes(t));
    ...
    .filter(
      (x) =>
        x.hits.length >= 2 ||
        commentKey.includes(
          matchTokens(x.feature.feature_name).slice(0, 3).join(" "),
        ),
    );
  const maxHits = Math.max(0, ...candidates.map((x) => x.hits.length));
  const best = candidates.filter((x) => x.hits.length === maxHits);
  return {
    match: best.length === 1 ? best[0].feature : null,
    matches: best.map((x) => x.feature),
  };
}
...
function normaliseStatus(v) {
  ...
  const aliases = {
    ...
    sit: "sit_in_progress",
    deploy: "deployment_in_progress",
    deployment: "deployment_in_progress",
    ba: "bs_signoff_in_progress",
    bs: "bs_signoff_in_progress",
    done: "uat_done",
    uat: "uat_in_progress",
  };
...
function buildWorkspaceMilestones(features, existing = {}) {
  ...
  features.forEach((f) => {
    ...
    if (!map[ws]["UAT Internal"] && f["UAT Internal"])
      map[ws]["UAT Internal"] = f["UAT Internal"];
  });
  return map;
}
function milestoneHealth(dueValue, features, target) {
  ...
  const critical = blockers.filter(
    (f) => priority(f.user_count) === "Critical",
  ).length;
  if (!due) return { label: "No date", tone: "neutral", blockers, critical };
  if (!blockers.length)
    return TODAY < due
      ? { label: "Ahead", tone: "ahead", blockers, critical }
      : { label: "Completed", tone: "completed", blockers, critical };
  if (TODAY > due)
    return { label: "Delayed", tone: "delayed", blockers, critical };
  if (due <= NEXT_SPRINT_END)
    return { label: "Due Next Sprint", tone: "due_next", blockers, critical };
...
function workspaceSummaries(features, milestones) {
  ...
  return Object.entries(byWs)
    .map(([workspace, items]) => {
      const dates = milestones[workspace] || {
        Build: "",
        SIT: "",
        "UAT Internal": "",
      };
      const ms = MILESTONES.map((m) => ({
        ...m,
        health: milestoneHealth(dates[m.key], items, m.target),
      }));
      const blockers = Array.from(
        new Map(
          ms.flatMap((m) => m.health.blockers).map((f) => [f.id, f]),
        ).values(),
      );
      ...
      const allMet = ms.every((m) =>
        ["ahead", "completed"].includes(m.health.tone),
      );
      const overallTone = delayed
        ? "delayed"
        : dueNext
          ? "due_next"
          : allAhead
      ...
      return {
        ...
        progress: items.length
          ? Math.round(
              items.reduce((sum, f) => sum + score(f.status), 0) / items.length,
            )
          : 0,
        totalUsers: items.reduce((s, f) => s + Number(f.user_count || 0), 0),
...
function defaultSprintDateRange(sprintId) {
  ...
  return {
    sprint: normaliseSprintName(sprintId),
...
function effectiveSprintDateRows(sprints, sprintDates) {
  ...
  return sprints.map((sprintId) => {
    ...
    return {
      ...
      ...custom,
      sprint: sprintKey,
      startDate: custom?.startDate || fallback.startDate,
      endDate: custom?.endDate || fallback.endDate,
    };
  });
}
function splitDaysOffBySprint(
  owner,
  startDate,
  endDate,
  sprintDateRows,
  ...
  return sprintDateRows.flatMap((row) => {
    ...
    if (
      !sprintStart ||
      !sprintEnd ||
      !dateRangesOverlap(start, end, sprintStart, sprintEnd)
    )
      return [];
      ...
      ? [
          {
            ...
            owner,
            sprint: row.sprint,
            startDate: toDateInputValue(overlapStart),
            endDate: toDateInputValue(overlapEnd),
            days,
            note,
          },
        ]
...
function workingDaysForMonthDays(
  startDay,
  endDay,
  monthName,
...
function daysOffCountFromText(value) {
  ...
  let total = 0;
  text = text.replace(
    /\b(\d{1,2})\s*(?:-|to)\s*(\d{1,2})\s*(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\b/g,
    (_, start, end, month) => {
  ...
  const dateList =
    /((?:\d{1,2}\s*(?:,|and|&)?\s*)+)\s*(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\b/g;
  let match;
...
function personDayOffEntries(comment, owners, fallbackDays) {
  ...
  const found = owners
    .map((owner) => {
      ...
      const match = text.match(new RegExp(`\\b${escaped}\\b\\s*:?`, "i"));
      return match
        ? { owner, index: match.index, end: match.index + match[0].length }
        : null;
    })
    .filter(Boolean)
  ...
  return found
    .map((entry, idx) => {
      ...
      const segment = text
        .slice(entry.end, next)
        .replace(/^[:\s-]+/, "")
        .trim();
      const days = daysOffCountFromText(segment);
      return {
        owner: entry.owner,
        days: days || Number(fallbackDays || 0),
        note: `${entry.owner}: ${segment || text}`,
      };
    })
...
function dueThisSprint(features, milestones) {
  ...
  const futureDueDates = incomplete
    .map((item) => item.due)
    .filter((d) => d > NEXT_SPRINT_END)
    .sort((a, b) => a - b);
  const sprintEnds = [CURRENT_SPRINT_END, NEXT_SPRINT_END, ...futureDueDates];
  const sprintEnd =
    sprintEnds.find((end) => incomplete.some((item) => item.due <= end)) ||
    null;
  const items = sprintEnd
    ? incomplete.filter((item) => item.due <= sprintEnd)
    : [];
  items.sort(
    (a, b) =>
      a.due - b.due ||
...
function groupDueSprintItems(items) {
  return Object.values(
    items.reduce((groups, item) => {
      ...
      if (!groups[workspace])
        groups[workspace] = { workspace, items: [], critical: 0 };
      groups[workspace].items.push(item);
      if (priority(item.feature.user_count) === "Critical")
        groups[workspace].critical += 1;
      return groups;
    }, {}),
  ).sort(
    (a, b) =>
      b.items.length - a.items.length || a.workspace.localeCompare(b.workspace),
  );
}
function timelineMonths(summaries) {
  ...
  summaries.forEach((ws) =>
    MILESTONES.forEach((m) => {
      const d = parseDate(ws.dates[m.key]);
      if (d) points.push(monthStart(d));
    }),
  );
  const min = points.reduce((a, b) => (a < b ? a : b));
  const max = points.reduce((a, b) => (a > b ? a : b));
  const out = [];
  let c = monthStart(min);
  while (c <= max) {
    out.push(new Date(c));
    c = addMonth(c, 1);
  }
...
function FeatureCard({ feature, density, onEdit, onDragStart, allocations }) {
  return (
    <div
      className={`feature-card ${density} ${priorityClass(feature.user_count)}`}
      draggable
    ...
    >
      ...
      <div className="card-meta">
        ...
        <span className="user-pill">{feature.user_count || 0} users</span>
      </div>
      <div className="card-meta">
        <span>{priority(feature.user_count)}</span>
        <span>{STATUS_LABEL[feature.status]}</span>
      </div>
      {density !== "compact" && nextStageLabel(feature.status) && (
        <div className="ready-tag">{nextStageLabel(feature.status)}</div>
      )}
      {planningBadge(feature.id, allocations) && (
...
function BoardView({
  ...
  const boardFeatures = features.filter(
    (f) =>
      (workspaceFilter === "ALL" || f.workspace === workspaceFilter) &&
      (ownerFilter === "ALL" || splitOwners(f.owner).includes(ownerFilter)),
  );
  const inFlight = boardFeatures.filter(
    (f) => !["initial", "uat_done"].includes(f.status),
  ).length;
  const readyNext = boardFeatures.filter((f) =>
    ["build_done", "sit_done", "deployment_done", "bs_signoff_done"].includes(
      f.status,
    ),
  ).length;
  function start(e, fid) {
  ...
  function drop(e, status) {
    ...
    setFeatures((prev) =>
      prev.map((f) => (f.id === fid ? { ...f, status } : f)),
    );
  }
  return (
    <>
...
function AddFeature({ workspaces, onAdd }) {
  ...
  useEffect(() => {
    if (!form.workspace)
      setForm((p) => ({
        ...p,
        workspace: workspaces.find((w) => w !== "ALL") || "",
      }));
  ...
  function add() {
    ...
    onAdd({
      ...
      workspace: form.workspace || "Unknown",
    ...
    <div className="panel">
      ...
      <div className="queue-grid">
        {buckets.map(([status, title]) => (
          <div className="queue" key={status}>
            <h4>{title}</h4>
            {features
              .filter((f) => f.status === status)
              .sort(
                (a, b) => Number(b.user_count || 0) - Number(a.user_count || 0),
              )
              .map((f) => (
                <div
                  className={`queue-card ${priorityClass(f.user_count)}`}
                  key={f.id}
                >
                  <div>
                    <b>{f.feature_name}</b>
                    <small>
                      {f.workspace} · {f.owner || "No owner"}
                    </small>
                  </div>
                  <div className="detail-card-actions">
                    <span>
                      {Number(f.user_count || 0).toLocaleString()} users
                    </span>
...
function ExecutiveDashboard({
  ...
  const filtered =
    workspaceFilter === "ALL"
      ? features
      : features.filter((f) => f.workspace === workspaceFilter);
  ...
  return (
    <div className="dashboard">
      ...
      <div className="summary-grid">
        ...
        <SummaryCard
          label="Delayed"
          value={summaries.filter((s) => s.overallTone === "delayed").length}
          hint="Missed milestone"
        />
        <SummaryCard
          label="Due Next Sprint"
          value={summaries.filter((s) => s.overallTone === "due_next").length}
          hint="Needs planning focus"
        />
        <SummaryCard
          label="Completed / Ahead"
          value={
            summaries.filter((s) =>
              ["completed", "ahead"].includes(s.overallTone),
            ).length
          }
        ...
        <SummaryCard
          ...
          value={summaries.reduce((n, s) => n + s.blockers.length, 0)}
          hint="Blocking milestones"
        />
        <SummaryCard
          label="Critical Blocking"
          value={summaries.reduce((n, s) => n + s.criticalBlockers, 0)}
      ...
      <div className="panel roadmap-panel">
        ...
        <div
          className="timeline-head exec-roadmap-head"
          style={{
            gridTemplateColumns: `280px 180px repeat(${months.length}, minmax(88px,1fr)) 150px`,
          }}
        >
          <span>Workspace</span>
          <span>Progress</span>
          {months.map((m, i) => (
            <span key={i} className={i === todayIdx ? "today-month" : ""}>
              {monthLabel(m)}
            </span>
          ))}
          <span>Health</span>
        ...
        {summaries.map((s) => (
          <div
            className="timeline-row exec-roadmap-row"
            style={{ gridTemplateColumns: `280px 180px 1fr 150px` }}
            key={s.workspace}
          ...
          >
            <div>
              <b>{s.workspace}</b>
              <small>
                {s.features.length} features · {s.blockers.length} blockers
              </small>
            </div>
            <div className="roadmap-progress">
              <div className="progress-track">
                <i
                  className={s.overallTone}
            ...
            <div
              ...
              style={{
                gridTemplateColumns: `repeat(${months.length}, minmax(88px,1fr))`,
              }}
            >
              {months.map((m, i) => (
                <div key={i} className={i === todayIdx ? "today-cell" : ""} />
              ))}
              {todayIdx >= 0 && (
                <div
                  className="today-line"
                  style={{
                    left: `calc(${((todayIdx + 0.5) / months.length) * 100}% - 1px)`,
                  }}
                />
              )}
              {s.milestones.map((m) => {
                const idx = monthIndex(parseDate(s.dates[m.key]), months);
                return idx < 0 ? null : (
                  <div
                    key={m.key}
                    className={`dot ${m.health.tone}`}
                    style={{
                      left: `calc(${((idx + 0.5) / months.length) * 100}% - 12px)`,
                    }}
                    title={`${m.label} ${fmtDate(s.dates[m.key])}`}
                  >
                    {m.short}
                  </div>
                );
      ...
      {editWs && (
        <WorkspaceModal
          workspace={editWs}
          dates={milestones[editWs]}
          onClose={() => setEditWs(null)}
          onSave={(ws, dates) => {
            setMilestones((p) => ({ ...p, [ws]: dates }));
            setEditWs(null);
          }}
        />
      )}
  ...
  if (!summary)
    return (
      <div className="panel">
        ...
        <p className="muted">Select a workspace to view blockers.</p>
      </div>
    );
  return (
    <div className="panel">
      <div className="panel-top">
        <div>
          ...
          <small>Blocking features by milestone</small>
        </div>
        <button onClick={onClose}>Close</button>
      </div>
      {summary.milestones.map((m) => {
        const ranked = m.health.blockers
          .slice()
          .sort(
            (a, b) => Number(b.user_count || 0) - Number(a.user_count || 0),
          );
        return (
          <div className="detail-section" key={m.key}>
            <div className="panel-top">
              <div>
                <b>{m.label}</b>
                <small>Due {fmtDate(summary.dates[m.key])}</small>
              </div>
            ...
            {ranked.length ? (
              ranked.map((f) => (
                ...
                >
                  <div>
                    <b>{f.feature_name}</b>
                    <small>
                      {f.owner || "Unassigned"} · {STATUS_LABEL[f.status]}
                    </small>
                  </div>
                  <div className="detail-card-actions">
                    <span>
              ...
              <p className="muted">No blocking features.</p>
            )}
          </div>
        );
      })}
...
function WorkspaceModal({ workspace, dates, onClose, onSave }) {
  const [form, setForm] = useState(
    dates || { Build: "", SIT: "", "UAT Internal": "" },
  );
  return (
    <div className="modal">
      <div className="modal-card small">
        <div className="panel-top">
          <h2>Edit workspace milestones</h2>
          <button onClick={onClose}>Close</button>
        </div>
        <h3>{workspace}</h3>
        {MILESTONES.map((m) => (
        ...
        <div className="modal-actions">
          ...
          <button onClick={() => onSave(workspace, form)}>Save</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="modal-card">
        ...
        <div className="form-grid">
          ...
          <label>
            ...
            <input
              type="number"
              value={form.user_count || 0}
              onChange={(e) =>
                setForm({ ...form, user_count: Number(e.target.value || 0) })
              }
            />
          </label>
          <label>
...
function OverviewDashboard({
  ...
  const grouped = Object.fromEntries(
    STATUSES.map(([k]) => [k, filtered.filter((f) => f.status === k).length]),
  );
  const total = filtered.length;
  const totalUsers = filtered.reduce(
    (s, f) => s + Number(f.user_count || 0),
    0,
  );
  const nextActions = filtered
    .filter((f) =>
      ["build_done", "sit_done", "deployment_done", "bs_signoff_done"].includes(
        f.status,
      ),
    ...
    <div className="dashboard">
      <div className="dash-head">
        <div>
          ...
          <h1>Stage health and next sprint focus</h1>
        </div>
        <select
          value={workspaceFilter}
          onChange={(e) => setWorkspaceFilter(e.target.value)}
      ...
      <div className="summary-grid">
        <SummaryCard label="Total Features" value={total} hint="Current view" />
        <SummaryCard
          label="In Flight"
          value={
            filtered.filter((f) => !["initial", "uat_done"].includes(f.status))
              .length
          }
          hint="Not initial or done"
        />
        <SummaryCard
          label="Ready Next"
          value={nextActions.length}
          hint="Ready for next stage"
        />
        <SummaryCard
          label="Total Users"
        ...
        <SummaryCard
          label="Done"
          value={grouped.uat_done || 0}
          hint="UAT done"
        />
        <SummaryCard
          label="Progress"
          value={`${total ? Math.round(filtered.reduce((s, f) => s + score(f.status), 0) / total) : 0}%`}
          hint="Weighted maturity"
        />
      </div>
      <div className="two-col overview-grid">
        <div className="panel due-panel">
          <div className="panel-top">
            <div>
              <h3>Due This Sprint</h3>
              <p className="muted">
                Sprint ending {fmtDateValue(dueSprint.sprintEnd)}
              </p>
            </div>
            <span className="pill-status due_next">
              {dueSprint.items.length} items
            </span>
          </div>
          {dueGroups.length ? (
            dueGroups.map((group) => (
              <div
                className="priority-block workspace-due-block"
                key={group.workspace}
              >
                <div className="workspace-due-head">
                  <div>
                    <b>{group.workspace}</b>
                    <small>
                      {group.items.length} blocking features · {group.critical}{" "}
                      critical
                    </small>
                  </div>
                  <span className="ready-tag">
                    {group.items[0].milestone.label}
                  </span>
                </div>
                {group.items.map(({ feature: f, milestone: m, due }) => (
                  <div
                    key={`${f.id}-${m.key}`}
                    className={`next-action-card ${priorityClass(f.user_count)}`}
                  >
                    <div className="next-action-main">
                      <b>{f.feature_name}</b>
                      <small>
                        {f.workspace} · {STATUS_LABEL[f.status]}
                      </small>
                      <small>
                        Milestone needed: {m.label} · Due {fmtDateValue(due)}
                      </small>
                    </div>
                    <div className="next-action-impact">
                      <span>
                        {Number(f.user_count || 0).toLocaleString()} users
                      </span>
                      <b>{priority(f.user_count)}</b>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <p className="muted">
              No incomplete milestone work has a due date.
            </p>
          )}
        </div>
          ...
          {STATUSES.map(([k, l]) => (
            <div className="bar-row" key={k}>
              ...
              <div>
                <i
                  style={{
                    width: `${total ? ((grouped[k] || 0) / total) * 100 : 0}%`,
                  }}
                />
              </div>
            </div>
        ...
        {nextActions.length ? (
          nextActions.map((f) => (
            ...
            >
              <div className="next-action-main">
                <b>{f.feature_name}</b>
                <small>{f.workspace}</small>
                <small>Current: {STATUS_LABEL[f.status]}</small>
                <small>Next: {nextStageLabel(f.status)}</small>
                <small>Due: {fmtDate(nextStageDueDate(f, milestones))}</small>
              </div>
              <div className="next-action-impact">
          ...
          <p className="muted">
            No features currently ready for the next action.
          </p>
        )}
      </div>
    </div>
...
function DeliveryPlan({
  ...
  const sprintOptions = useMemo(() => {
    const fromData = Array.from(
      new Set(
        [
          ...customSprints,
          ...capacities.map((c) => c.sprint),
          ...allocations.map((a) => a.sprint),
        ]
          .map((s) => normaliseSprintName(s))
          .filter(Boolean),
      ),
    );
  ...
  const [prodStoryDraft, setProdStoryDraft] = useState({
    name: "",
    devOpsId: "",
    notes: "",
  });
  ...
  useEffect(() => {
    setQuickPlan((q) => ({
      ...q,
      sprint: q.sprint || sprintOptions[0] || "26Q1S1",
  ...
  const mappingFeatures = useMemo(
    () => [...features, ...prodSupportFeatures],
    [features, prodSupportFeatures],
  );
  const allFeatureWorkspaces = useMemo(
    () =>
      Array.from(
        new Set(
          [
            ...features.map((f) => f.workspace),
            ...prodSupportFeatures.map((f) => f.workspace),
          ].filter(Boolean),
        ),
      ).sort(),
    [features, prodSupportFeatures],
    ...
    allocations.forEach((a) =>
      resolvedAllocationFeatureIds(a).forEach((featureId) => {
        ...
        map
          ...
          .push({
            ...a,
            visibleFeatureId: featureId,
            isStageComplete: allocationFeatureComplete(a, featureId),
          });
      }),
    );
    return map;
  ...
  const standaloneAllocationsByKey = useMemo(() => {
    ...
    allocations.forEach((a) => {
      if (
        ...
        return;
      const key = a.featureName || a.planningGroup || a.workspaceName;
      if (!key) return;
      if (!map.has(key)) map.set(key, []);
      map.get(key).push(a);
    });
  ...
  const planningFeatures = useMemo(
    () => [
      ...
      ...standaloneKeys.map((key) => ({
        ...
        planningKey: key,
        feature_name: key,
        workspace: standaloneAllocationsByKey.get(key)?.[0]?.workspace || key,
        status: "initial",
        owner: "",
        user_count: 0,
      })),
  ...
  const entityAllocations = (feature) =>
    feature.planningKey
      ? standaloneAllocationsByKey.get(feature.planningKey) || []
      : allocationsByFeatureId.get(feature.id) || [];
  const featureFinalStage = (feature) =>
    finalStageByFeatureId?.[feature.id]
      ? normaliseFinalStage(finalStageByFeatureId[feature.id])
      : defaultFinalStageForFeature(feature);
  const updateFeatureFinalStage = (featureId, stage) =>
    setFinalStageByFeatureId((prev) => ({
      ...prev,
      [featureId]: normaliseFinalStage(stage),
    }));
  const entityThrough = (feature) => {
    const rows = entityAllocations(feature);
    const actualIdx = PLAN_STAGES.indexOf(
      actualCompletedThrough(feature.status),
    );
    ...
    const max = Math.max(
      actualIdx,
      completed.length ? Math.max(...completed) : -1,
    );
    return max >= 0 ? PLAN_STAGES[max] : "Not planned";
  };
  const entityIsPlanningComplete = (feature) =>
    PLAN_STAGES.indexOf(entityThrough(feature)) >=
    finalStageIndex(featureFinalStage(feature));
  const entityLatestPlannedStage = (feature) => {
    if (entityIsPlanningComplete(feature)) return "Planning Complete";
    const rows = entityAllocations(feature);
    const floor = boardPlanningFloor(feature.status);
    if (floor === "Planning Complete") return floor;
    const targetIdx = finalStageIndex(featureFinalStage(feature));
    ...
    const max = Math.min(
      ...
      Math.max(
        floorIdx,
        plannedOnly.length ? Math.max(...plannedOnly) : -1,
        completedNext.length ? Math.max(...completedNext) : -1,
      ),
    );
    if (max >= 0) return PLAN_STAGES[max];
    return rows.some((a) => a.stage === NEEDS_MAPPING_STAGE)
      ? NEEDS_MAPPING_STAGE
      : "Requirement";
  };
  const entityNext = (feature) =>
    nextPlanningStageFromRows(
      feature,
      entityAllocations(feature),
      featureFinalStage(feature),
    );
  const entityLatestSprint = (feature) =>
    entityAllocations(feature)
      .map((a) => a.sprint)
      .filter(Boolean)
      .sort()
  ...
  const entityMilestones = (feature) =>
    STAGE_OPTIONS.map((stage) => {
      const rows = entityAllocations(feature)
        .filter((a) => a.stage === stage && a.sprint)
        .sort((a, b) => String(a.sprint).localeCompare(String(b.sprint)));
      const latest = rows.at(-1);
      return latest
        ? {
            stage,
            sprint: latest.sprint,
            sourceStage: latest.sourceStage,
            complete: !!latest.isStageComplete,
          }
        : null;
    }).filter(Boolean);
  const ownerSprintKey = (owner, sprintId) =>
    `${normalisePersonName(owner)}||${normaliseSprintName(sprintId)}`;
  const capacityByOwnerSprint = useMemo(() => {
    const map = new Map();
    capacities.forEach((c) => map.set(ownerSprintKey(c.owner, c.sprint), c));
    return map;
  }, [capacities]);
  const plannedByOwnerSprint = useMemo(() => {
    const map = new Map();
    allocations.forEach((a) => {
      const key = ownerSprintKey(a.owner, a.sprint);
      map.set(key, (map.get(key) || 0) + Number(a.days || 0));
    });
    return map;
  }, [allocations]);
  const effectiveSprintDates = useMemo(
  ...
  const daysOffByOwnerSprint = useMemo(() => {
    const map = new Map();
    daysOff.forEach((row) => {
      const owner = normalisePersonName(row.owner);
      const sprintId =
        normaliseSprintName(row.sprint) ||
        sprintForDateRange(row.startDate, row.endDate, effectiveSprintDates);
      const days = Number(
        row.days || workingDaysBetween(row.startDate, row.endDate) || 0,
      );
      if (owner && sprintId && days > 0) {
        const key = ownerSprintKey(owner, sprintId);
        map.set(key, (map.get(key) || 0) + days);
      }
    });
    return map;
  }, [daysOff, effectiveSprintDates]);
  const visibleOwners =
    ownerFilter === "ALL"
      ? ownerOptions
      : ownerOptions.filter((o) => o === ownerFilter);
  const capacityRows = visibleOwners.map((owner) => {
    const ownerKey = normalisePersonName(owner);
    const key = ownerSprintKey(ownerKey, sprint);
    ...
    const planned = plannedByOwnerSprint.get(key) || 0;
    return {
      owner: ownerKey,
      base,
      off,
      available,
      planned,
      remaining: available == null ? null : available - planned,
      counted: [],
    };
  });
      ...
      allocations.flatMap((a) => {
        ...
        if (ids.length <= 1) {
          ...
          return [
            {
              ...a,
              sourceAllocationId: allocationSourceId(a),
              visibleFeatureId: featureId,
              visibleFeatureName:
                featureById.get(featureId)?.feature_name ||
                allocationDisplayName(a),
              isStageComplete: allocationFeatureComplete(a, featureId),
            },
          ];
        }
        return ids.map((featureId) => {
          ...
          const visibleName =
            ...
            String(a.featureName || a.actualFeatureName || "")
              .split(" / ")
              .find(Boolean) ||
            allocationDisplayName(a);
          return {
            ...
            sourceAllocationId: allocationSourceId(a),
            visibleFeatureId: featureId,
            visibleFeatureName: visibleName,
            isStageComplete: allocationFeatureComplete(a, featureId),
          };
        });
      }),
  ...
  function updateCapacity(owner, availableDays) {
    const ownerKey = normalisePersonName(owner);
    const sprintKey = normaliseSprintName(sprint);
    setCapacities((prev) => {
      const rest = prev.filter(
        (c) =>
          !(
            normaliseSprintName(c.sprint) === sprintKey &&
            normalisePersonName(c.owner) === ownerKey
          ),
      );
      return [
        ...rest,
  ...
  function addDayOff() {
    ...
    if (!rows.length) {
      setDayOffWarning(
        "No sprint date range matched that leave date. Check Sprint date setup.",
      );
      return;
    }
    setDaysOff((prev) => [...rows, ...prev]);
    setDayOffWarning("");
  ...
  function updateSprintDates(sprintId, patch) {
    ...
    setSprintDates((prev) => {
      const rest = prev.filter(
        (row) => normaliseSprintName(row.sprint) !== sprintKey,
      );
      const current = prev.find(
        (row) => normaliseSprintName(row.sprint) === sprintKey,
      ) || { sprint: sprintKey, startDate: "", endDate: "" };
      return [...rest, { ...current, ...patch, sprint: sprintKey }].sort(
        (a, b) => a.sprint.localeCompare(b.sprint),
      );
    });
  }
  function updateAllocation(allocationId, patch) {
    setAllocations((prev) =>
      prev.map((a) =>
        a.id === allocationId
          ? {
              ...a,
              ...patch,
              stage: patch.stage ? normalisePlanStage(patch.stage) : a.stage,
              owner: patch.owner ? normalisePersonName(patch.owner) : a.owner,
              sprint: patch.sprint
                ? normaliseSprintName(patch.sprint)
                : a.sprint,
            }
          : a,
      ),
    ...
    setAllocations((prev) => [
      {
        ...source,
        id: id(),
        sourceAllocationId: sourceId,
        featureId: source.featureId || "",
        featureIds: source.featureId ? [source.featureId] : [],
        matchedFeatureIds: source.featureId ? [source.featureId] : [],
        owner: "",
        days: 0,
        sharedVisibility: false,
        isStageComplete: false,
        childOutcomes: source.featureId ? { [source.featureId]: false } : {},
      },
      ...prev,
    ]);
  }
  function deletePlanningLine(lineId) {
    if (confirm("Delete this planning line?"))
      setAllocations((prev) => prev.filter((a) => a.id !== lineId));
  }
  function deleteAllocationSource(sourceId) {
    if (confirm("Delete this source allocation and all of its planning lines?"))
      setAllocations((prev) =>
        prev.filter((a) => allocationSourceId(a) !== sourceId),
      );
  }
  function updateChildOutcome(allocationId, featureId, isComplete) {
    setAllocations((prev) =>
      prev.map((a) => {
        if (a.id !== allocationId) return a;
        const ids = allocationFeatureIds(a);
        const complete = !!isComplete;
        const keepSourceAligned = ids.length <= 1 || featureId === a.featureId;
        return {
          ...a,
          isStageComplete: keepSourceAligned ? complete : a.isStageComplete,
          childOutcomes: { ...(a.childOutcomes || {}), [featureId]: complete },
        };
      }),
    );
  }
  function allocationOutcomeFeatureId(allocation) {
    const ids = allocationFeatureIds(allocation);
    return allocation.visibleFeatureId || (ids.length === 1 ? ids[0] : "");
  }
  function allocationOutcomeComplete(allocation) {
    const featureId = allocationOutcomeFeatureId(allocation);
    return featureId
      ? allocationFeatureComplete(allocation, featureId)
      : !!allocation.isStageComplete;
  }
  function updateAllocationOutcome(allocation, isComplete) {
    const featureId = allocationOutcomeFeatureId(allocation);
    const allocationId = allocation.sourceAllocationId || allocation.id;
    if (featureId) updateChildOutcome(allocationId, featureId, isComplete);
    else updateAllocation(allocationId, { isStageComplete: !!isComplete });
  }
  function linkAllocationToFeature(allocationId, feature) {
    if (!feature) return;
    const allocation = allocationById.get(allocationId) || {};
    updateAllocation(allocationId, {
      featureId: feature.id,
      featureIds: [feature.id],
      matchedFeatureIds: [feature.id],
      childOutcomes: { [feature.id]: !!allocation.isStageComplete },
      actualFeatureName: feature.feature_name,
      featureName: feature.feature_name,
      workspace: feature.workspace,
      mappingStatus: "matched",
    });
  }
  function updateAllocationFeature(allocationId, featureId) {
    if (featureId === CREATE_PROD_STORY_VALUE) {
      const allocation = allocationById.get(allocationId);
      createProdSupportStoryForAllocation(
        allocationId,
        allocation?.stageComment || "",
      );
      return;
    }
    linkAllocationToFeature(allocationId, featureById.get(featureId));
  }
  function createProdSupportStory(seed = {}) {
    const name = String(seed.name || prodStoryDraft.name || "").trim();
    if (!name) return null;
    const story = {
      id: id(),
      name,
      devOpsId: String(seed.devOpsId || prodStoryDraft.devOpsId || "").trim(),
      notes: String(seed.notes || prodStoryDraft.notes || "").trim(),
    };
    setProdSupportStories((prev) => [story, ...prev]);
    setProdStoryDraft({ name: "", devOpsId: "", notes: "" });
    return prodSupportStoryFeature(story);
  }
  function createProdSupportStoryForAllocation(allocationId, comment = "") {
    const parts = commentStoryParts(comment);
    const feature = createProdSupportStory({
      name: parts.name,
      devOpsId: parts.devOpsId,
      notes: comment,
    });
    if (feature) linkAllocationToFeature(allocationId, feature);
  }
  function updateDiagnosticFeatureObjects(
    diagnosticIndex,
    allocationId,
    selected,
  ) {
      ...
      prev.map((a) => {
        ...
        if (!selected.length)
          return {
            ...
            childOutcomes: {},
            actualFeatureName: "",
            featureName: a.planningGroup ? "" : a.featureName,
            workspace: a.planningGroup ? "" : a.workspace,
            mappingStatus: "manual",
          };
        const previous = a.childOutcomes || {};
        const childOutcomes = Object.fromEntries(
          selected.map((f, idx) => [
            f.id,
            Object.prototype.hasOwnProperty.call(previous, f.id)
              ? !!previous[f.id]
              : f.id === a.featureId
                ? !!a.isStageComplete
                : idx === 0 && !!a.isStageComplete,
          ]),
        );
        return {
          ...a,
          featureId: selected[0].id,
          featureIds: selected.map((f) => f.id),
          matchedFeatureIds: selected.map((f) => f.id),
          childOutcomes,
          actualFeatureName: selected.map((f) => f.feature_name).join(" / "),
          featureName: selected.map((f) => f.feature_name).join(" / "),
          workspace: selected[0].workspace,
          mappingStatus: "matched",
        };
      }),
    );
    setImportDiagnostics((prev) =>
      prev.map((row, idx) =>
        idx === diagnosticIndex
          ? {
              ...row,
              matchedFeature: selected.map((f) => f.feature_name).join(" | "),
              mappingStatus: selected.length ? "matched" : "manual",
            }
          : row,
      ),
    );
  }
  function handleDiagnosticFeatureSelect(row, value) {
    if (value === CREATE_PROD_STORY_VALUE) {
      const feature = createProdSupportStory(commentStoryParts(row.comment));
      if (feature) {
        const current = allocationFeatureIds(
          allocationById.get(row.allocationId) || {},
        )
          .map((featureId) => featureById.get(featureId))
          .filter(Boolean);
        updateDiagnosticFeatureObjects(row.diagnosticIndex, row.allocationId, [
          ...current,
          feature,
        ]);
      }
    ...
    addDiagnosticFeature(row.diagnosticIndex, row.allocationId, value);
  }
  function updateDiagnosticFeatures(diagnosticIndex, allocationId, featureIds) {
    const cleanIds = Array.from(new Set(featureIds.filter(Boolean)));
    updateDiagnosticFeatureObjects(
      diagnosticIndex,
      allocationId,
      cleanIds.map((featureId) => featureById.get(featureId)).filter(Boolean),
    );
  }
  function addDiagnosticFeature(diagnosticIndex, allocationId, featureId) {
    if (!featureId) return;
    updateDiagnosticFeatures(diagnosticIndex, allocationId, [
      ...allocationFeatureIds(allocationById.get(allocationId) || {}),
      featureId,
    ]);
  }
  function removeDiagnosticFeature(diagnosticIndex, allocationId, featureId) {
    updateDiagnosticFeatures(
      diagnosticIndex,
      allocationId,
      allocationFeatureIds(allocationById.get(allocationId) || {}).filter(
        (id) => id !== featureId,
      ),
    );
  }
  function deleteAllocation(allocationId) {
    deletePlanningLine(allocationId);
  }
  function saveRecommendedAllocation(feature) {
    const stage = normalisePlanStage(quickPlan.stage) || entityNext(feature);
    if (stage === "Planning Complete") return false;
    const sprintToUse = normaliseSprintName(
      quickPlan.sprint === "ALL" ? sprintOptions[0] : quickPlan.sprint,
    );
    const owner = normalisePersonName(quickPlan.owner || ownerOptions[0] || "");
    const days = Number(quickPlan.days || 0);
    const remaining = remainingFor(owner, sprintToUse);
    setWarning(
      remaining != null && days > remaining
        ? `${owner} will be over capacity by ${days - remaining} day(s) in ${sprintToUse}.`
        : "",
    );
    const base = {
      id: id(),
      ...
      owner,
      days,
      isStageComplete: !!quickPlan.isStageComplete,
    };
    setAllocations((prev) => [
      feature.planningKey
        ? { ...base, planningGroup: "", workspaceName: feature.planningKey }
        : { ...base, featureId: feature.id },
      ...prev,
    ]);
  ...
  function addSprint() {
    const name = normaliseSprintName(newSprint);
    if (!name) return;
    setCustomSprints((prev) =>
      Array.from(new Set([...prev.map(normaliseSprintName), name])).sort(),
    );
    setSprint(name);
    setQuickPlan((q) => ({ ...q, sprint: name }));
    setNewSprint("");
  }
  function exportPlanningData() {
    ...
    a.href = url;
    a.download = "planning-kanban-data.json";
    a.click();
    URL.revokeObjectURL(url);
  }
  function importPlanningData(e) {
    ...
    reader.onload = () => {
      try {
        ...
        if (!Array.isArray(data.allocations) || !Array.isArray(data.capacities))
          throw new Error(
            "File does not contain planning allocations and capacities.",
          );
        ...
        const restoredAllocations = data.allocations.flatMap((a) => {
          ...
          return expandSharedAllocation(rebindImportedAllocation({
            ...
            stage:
              normalisePlanStage(sourceStage) ||
              normalisePlanStage(a.stage) ||
              NEEDS_MAPPING_STAGE,
            sourceStage,
        ...
        const restoredCapacities = data.capacities.map((c) => ({
          ...c,
          sprint: normaliseSprintName(c.sprint),
          owner: normalisePersonName(c.owner),
          availableDays: Number(c.availableDays || 0),
        }));
        const restoredDaysOff = Array.isArray(data.daysOff)
          ? data.daysOff.map((row) => ({
              ...row,
              id: row.id || id(),
              owner: normalisePersonName(row.owner),
          ...
          : [];
        const restoredSprintDates = Array.isArray(data.sprintDates)
          ? data.sprintDates.map((row) => ({
              ...row,
              sprint: normaliseSprintName(row.sprint),
            }))
          : [];
        const restoredProdStories = Array.isArray(data.prodSupportStories)
          ? data.prodSupportStories
              .map((row) => ({
                ...row,
                id: row.id || id(),
        ...
        const nextSprint =
          Array.from(
            new Set(
              [
                ...restoredSprints,
                ...restoredCapacities.map((c) => c.sprint),
                ...restoredAllocations.map((a) => a.sprint),
              ].filter(Boolean),
            ),
          ).sort()[0] || "26Q1S1";
        setAllocations(restoredAllocations);
        setCapacities(restoredCapacities);
        setDaysOff(restoredDaysOff);
        setSprintDates(restoredSprintDates);
        setProdSupportStories(restoredProdStories);
        setFinalStageByFeatureId(restoredFinalStages);
        setCustomSprints(restoredSprints);
  ...
  async function importPlanningExcel(e) {
    ...
    try {
      ...
      const wb = XLSX.read(workbookBuffer, {
        type: "array",
        cellStyles: true,
        bookFiles: true,
      });
      ...
      if (!sheet) {
        setImportMessage('Sheet "26Q1 Planner" was not found.');
        return;
      }
      ...
      const normaliseFillColor = (color) => {
        if (!color) return "";
        if (typeof color === "string")
          return color.toUpperCase().replace(/^FF([0-9A-F]{6})$/, "$1");
        if (color.rgb)
          return String(color.rgb)
            .toUpperCase()
            .replace(/^FF([0-9A-F]{6})$/, "$1");
        if (color.indexed != null)
          return (
            excelIndexedColors[color.indexed] || `INDEXED:${color.indexed}`
          );
        if (color.theme != null)
          return `THEME:${color.theme}:${color.tint ?? ""}`;
        return "";
      };
      const isExpectedCompleteColor = (color) =>
        ["FFFF00", "92D050"].includes(normaliseFillColor(color));
      const fileText = (name) => {
        const fileEntry = wb.files?.[name] || wb.files?.[`/${name}`];
        const content = fileEntry?.content ?? fileEntry?.data ?? fileEntry;
        if (!content) return "";
        if (typeof content === "string") return content;
        return new TextDecoder().decode(content);
      };
      const parseXml = (text) =>
        text ? new DOMParser().parseFromString(text, "application/xml") : null;
      const cleanCommentText = (value) =>
        String(value || "")
          .replace(/\r/g, "\n")
          .replace(/\[Threaded comment\][\s\S]*?Comment:\s*/i, "")
          .replace(/\s+/g, " ")
          .trim();
      const commentCompareKey = (value) =>
        String(value || "")
          .replace(/&amp;/gi, "&")
          .replace(/&lt;/gi, "<")
          .replace(/&gt;/gi, ">")
      ...
      const mergeCommentText = (...values) => {
        ...
        return values
          ...
          .filter(Boolean)
          .filter((txt) => {
            const key = commentCompareKey(txt);
      ...
      const xmlCommentMap = (() => {
        try {
          const comments = {};
          Object.keys(wb.files || {}).forEach((name) => {
            const rawName = String(name);
            const cleanName = rawName.startsWith("/")
              ? rawName.slice(1)
              : rawName;
            if (
              cleanName.startsWith("xl/comments") &&
              cleanName.endsWith(".xml")
            ) {
              const xml = parseXml(fileText(cleanName));
              xml?.querySelectorAll("comment").forEach((node) => {
                const ref = node.getAttribute("ref");
                const txt = cleanCommentText(
                  Array.from(node.querySelectorAll("t"))
                    .map((t) => t.textContent || "")
                    .join(" "),
                );
                if (ref && txt)
                  comments[ref] = mergeCommentText(comments[ref], txt);
              });
            }
            if (
              cleanName.startsWith("xl/threadedComments/threadedComment") &&
              cleanName.endsWith(".xml")
            ) {
              const xml = parseXml(fileText(cleanName));
              xml?.querySelectorAll("threadedComment").forEach((node) => {
                const ref = node.getAttribute("ref");
                const txt = cleanCommentText(
          ...
          console.warn("Planning Excel comment diagnostics failed", err);
          return {};
        }
      })();
      const stageComment = (r, c) => {
        const ref = XLSX.utils.encode_cell({ r, c });
        const fromXml = xmlCommentMap[ref] || "";
        const fromSheet = (rawCell(r, c)?.c || [])
          .map((c) => c.t || c.a || "")
          .join(" | ");
        return mergeCommentText(fromXml, fromSheet);
      };
      const xmlFillMap = (() => {
        try {
          const workbookXml = parseXml(fileText("xl/workbook.xml"));
          const relsXml = parseXml(fileText("xl/_rels/workbook.xml.rels"));
          const stylesXml = parseXml(fileText("xl/styles.xml"));
          if (!workbookXml || !relsXml || !stylesXml) return null;
          const relTargetById = Object.fromEntries(
            Array.from(relsXml.querySelectorAll("Relationship")).map((rel) => [
              rel.getAttribute("Id"),
              rel.getAttribute("Target"),
            ]),
          );
          const sheetNode = Array.from(
            workbookXml.querySelectorAll("sheet"),
          ).find((node) => node.getAttribute("name") === "26Q1 Planner");
          const relId = sheetNode?.getAttribute("r:id");
          const target = relTargetById[relId];
          if (!target) return null;
          const cleanTarget = target.replace(/^\/xl\//, "").replace(/^\//, "");
          const sheetPath = cleanTarget.startsWith("xl/")
            ? cleanTarget
            : `xl/${cleanTarget}`;
          const sheetXml = parseXml(fileText(sheetPath));
          if (!sheetXml) return null;
          const fillValues = Array.from(
            stylesXml.querySelectorAll("fills > fill"),
          ).map((fill) => {
            const fg = fill.querySelector("patternFill > fgColor");
            const bg = fill.querySelector("patternFill > bgColor");
            return (
              normaliseFillColor({
          ...
          const fillIdByStyle = Array.from(
            stylesXml.querySelectorAll("cellXfs > xf"),
          ).map((xf) => Number(xf.getAttribute("fillId") || 0));
          const fills = {};
          sheetXml.querySelectorAll("c").forEach((node) => {
            const ref = node.getAttribute("r");
            const styleIdx = Number(node.getAttribute("s") || 0);
            const fillId = fillIdByStyle[styleIdx] || 0;
            const color = fillValues[fillId] || "";
            if (ref && color) fills[ref] = color;
          });
          return fills;
        } catch (err) {
          console.warn("Planning Excel XML fill diagnostics failed", err);
          return null;
        }
      })();
      const styleFillColors = (r, c) => {
        const style = rawCell(r, c)?.s || {};
        ...
        return [
          ...
          .map(normaliseFillColor)
          .filter(Boolean);
      };
      const fillColors = (r, c) => {
        const ref = XLSX.utils.encode_cell({ r, c });
        return Array.from(
          new Set([
            ...(xmlFillMap?.[ref] ? [xmlFillMap[ref]] : []),
            ...styleFillColors(r, c),
          ]),
        );
      };
      const hasReadableStyle = (r, c) =>
        [c, c + 1, c + 2].some(
          (col) =>
            fillColors(r, col).length ||
            Object.keys(rawCell(r, col)?.s || {}).length > 0,
        );
      const expectedComplete = (r, c) =>
        [c, c + 1, c + 2].some((col) =>
          fillColors(r, col).some(isExpectedCompleteColor),
        );
      const sprintCols = Array.from({ length: 7 }, (_, i) => ({
        label: `S${i + 1}`,
        col: 17 + i * 3,
        sprint: normaliseSprintName(`26Q1S${i + 1}`),
      }));
      const ownersFromComment = (comment) =>
      ...
      for (let r = 12; r <= range.e.r; r++) {
        ...
        if (
          /^name$/i.test(text(r, 17)) ||
          /^total$/i.test(text(r, 17)) ||
          /^capacity$/i.test(text(r, 18))
        )
          continue;
        sprintCols.forEach(({ sprint, col }) => {
          ...
          if (
            ...
            !/^total$/i.test(owner) &&
            days &&
            !isRolePlaceholder(owner)
          ) {
            const allocationFillColors =
              Array.from(
                new Set(
                  [col, col + 1, col + 2].flatMap((fillCol) =>
                    fillColors(r, fillCol),
                  ),
                ),
              ).join(", ") || "none";
            ...
            const exactMatch = !split
              ? features.find(
                  (f) =>
                    f.feature_name.trim().toLowerCase() ===
                    currentFeature.toLowerCase(),
                )
              : null;
            const comment = stageComment(r, col + 1);
            const prodSupportMatch =
              split && isProdSupportGroup(currentFeature) && comment
                ? (() => {
                    const parts = commentStoryParts(comment);
                    if (!parts.name) return null;
                    let story = [
                      ...prodSupportStories,
                      ...pendingProdSupportStories,
                    ].find((st) => matchKey(st.name) === matchKey(parts.name));
                    if (!story) {
                      story = {
                        id: id(),
                        name: parts.name,
                    ...
                    if (
                      !pendingProdSupportStories.some(
                        (st) => matchKey(st.name) === matchKey(story.name),
                      )
                    )
                      pendingProdSupportStories.push(story);
                    return prodSupportStoryFeature(story);
                  })()
                : null;
            const commentMatch =
              split && comment && !prodSupportMatch
                ? findFeatureFromComment(
                    comment,
                    currentFeature,
                    mappingFeatures,
                  )
                : { match: null, matches: [] };
            const matchedFeatures = prodSupportMatch
              ? [prodSupportMatch]
              : commentMatch.matches.length
                ? commentMatch.matches
                : commentMatch.match
                  ? [commentMatch.match]
                  : exactMatch
                    ? [exactMatch]
            ...
            const possibleMatches = matchedFeatures.length
              ? matchedFeatures.map((f) => f.feature_name)
              : commentMatch.matches.map((f) => f.feature_name);
            const commentHasGoal = /\bgoal\b/i.test(comment);
            const allocationExpectedComplete =
              !!isStageComplete || commentHasGoal;
            const childOutcomes = Object.fromEntries(
              matchedFeatures.map((f, idx) => [
                f.id,
                commentHasGoal ? idx === 0 : allocationExpectedComplete,
              ]),
            );
            const mappingStatus = matchedFeatures.length
              ? "matched"
              : comment
                ? "unmatched"
                : "manual";
            ...
            diagnostics.push({
              allocationId,
              row: r + 1,
              stageCell: XLSX.utils.encode_cell({ r, c: col + 1 }),
              sprint,
              person: owner,
              feature: currentFeature,
              stage: sourceStage || stage,
              days,
              fillColor: allocationFillColors,
              expectedComplete: allocationExpectedComplete,
              comment,
              matchedFeature: matchedFeatures
                .map((f) => f.feature_name)
                .join(" | "),
              possibleMatches: possibleMatches.join(" | "),
              mappingStatus,
            });
            const sourceAllocation = {
              ...
              matchedFeatureIds: matchedFeatures.map((f) => f.id),
              childOutcomes,
              featureName: matchedFeatures.length
                ? matchedFeatures.map((f) => f.feature_name).join(" / ")
                : split
                  ? ""
                  : currentFeature,
              actualFeatureName: matchedFeatures
                ...
                .join(" / "),
              planningGroup: split
                ? normaliseWorkspaceName(currentFeature)
                : "",
              workspaceName: normaliseWorkspaceName(currentFeature),
              workspace: normaliseWorkspaceName(
                match?.workspace || (!split ? currentFeature : ""),
              ),
              sprint,
              owner,
              originalOwner: owner,
              ...
              sourceDays: days,
              isStageComplete:
                matchedFeatures.length === 1
                  ? !!childOutcomes[match.id]
                  : allocationExpectedComplete,
            };
            imported.push(...expandSharedAllocation(sourceAllocation));
          }
        });
      }
      ...
      for (let r = 5; r <= 8; r++) {
        ...
        if (days > 0 && comment) {
          ...
          personDayOffEntries(comment, matchedOwners, days).forEach((row) =>
            importedDaysOff.push({
              id: id(),
              owner: row.owner,
              sprint: "26Q1S1",
              days: row.days,
      ...
      if (capacityStart >= 0) {
        for (let r = capacityStart + 1; r <= range.e.r; r++) {
          const hasAnyCapacity = sprintCols.some(
            ({ col }) => text(r, col) || cell(r, col + 1),
          );
      ...
      setProdSupportStories(() => {
        const byKey = new Map();
        pendingProdSupportStories.forEach((st) => {
          const key = matchKey(st.name);
          if (key && !byKey.has(key)) byKey.set(key, st);
        });
      ...
      const styleWarning =
        imported.length && !styledAllocationCount
          ? " Background colours could not be read; expected-complete markers were not imported. Use the Stage Outcome field to mark expected complete manually."
          : styledAllocationCount && !expectedCompleteCount
            ? " No yellow or green expected-complete fills were detected. Use the Stage Outcome field to mark expected complete manually if needed."
            : ` Imported ${expectedCompleteCount} expected-complete allocation(s) from yellow/green fills.`;
      const commentedCount = diagnostics.filter((d) => d.comment).length;
      const autoMatchedCount = diagnostics.filter(
        (d) => d.matchedFeature,
      ).length;
      const commentMessage = commentedCount
        ? ` Read ${commentedCount} stage-cell comment(s) and auto-linked ${autoMatchedCount} allocation(s) to child features.`
        : " No stage-cell comments were available to the browser parser; manual child-feature mapping remains available.";
      setImportMessage(
        `Imported ${imported.length} allocation(s). Imported ${importedCaps.length} capacity row(s). Imported ${importedDaysOff.length} day-off row(s).${styleWarning}${commentMessage}`,
      );
    } catch (err) {
      setImportMessage(`Excel import failed: ${err.message}`);
    } finally {
      e.target.value = "";
    }
  }
  const kanbanBase = planningFeatures.filter(
    (f) =>
      (planWorkspaceFilter === "ALL" || f.workspace === planWorkspaceFilter) &&
      (ownerFilter === "ALL" ||
  ...
  const kanbanLabels = {
    ...
    "BA Sign Off": "BA Sign Off Planning",
    UAT: "UAT Planning",
    "Planning Complete": "Planning Complete",
  };
  const kanbanColumns = [...PLAN_STAGES, "Planning Complete"].map((stage) => ({
    stage,
    label: kanbanLabels[stage],
    items: kanbanItems.filter((item) => item.latestStage === stage),
  }));
  const selectedAllocations = selectedPlanFeature
    ? entityAllocations(selectedPlanFeature).sort(
        (a, b) =>
  ...
  const visibleDaysOff = daysOff
    .filter(
      (row) =>
        ...
        ) === normaliseSprintName(sprint),
    )
    .slice()
    .sort(
      (a, b) =>
        normalisePersonName(a.owner).localeCompare(
          normalisePersonName(b.owner),
        ) || String(a.startDate || "").localeCompare(String(b.startDate || "")),
    );
  const openPlanFeature = (feature) => {
    const stage = entityNext(feature);
  ...
  const allocationNeedsMapping = (a) => {
    const ids = allocationFeatureIds(a);
    if (a.stage === NEEDS_MAPPING_STAGE) return true;
    if (ids.length > 0) return false;
    return isProdSupportGroup(
  ...
  const allocationMatrixRows = Object.values(
    filteredAllocations.reduce((groups, a) => {
      const item = a.visibleFeatureName || allocationDisplayName(a);
      const key = `${item}||${allocationWorkspace(a)}`;
      groups[key] ||
        (groups[key] = {
          key,
          item,
      ...
      groups[key].needsMapping =
        groups[key].needsMapping || allocationNeedsMapping(a);
      groups[key].days += Number(a.days || 0);
      return groups;
    }, {}),
  ).sort(
    (a, b) =>
  ...
  const editingAllocationRow = editingAllocation
    ...
    : null;
  const reconciliationLines = editingAllocationRow
    ? allocations.filter(
        (a) =>
          allocationSourceId(a) === allocationSourceId(editingAllocationRow),
      )
    : [];
  const reconciledDays = reconciliationLines.reduce(
    (sum, line) => sum + Number(line.days || 0),
    0,
  );
  const sprintPlanRows = allocations
    .filter(
      (a) =>
        (ownerFilter === "ALL" || a.owner === ownerFilter) &&
        (planWorkspaceFilter === "ALL" ||
  ...
  const sprintPlanGroups = Object.values(
    sprintPlanRows.reduce((groups, row) => {
      groups[row.sprint] ||
        (groups[row.sprint] = { sprint: row.sprint, rows: [], days: 0 });
      groups[row.sprint].rows.push(row);
      groups[row.sprint].days += row.days;
      return groups;
    }, {}),
  );
  ...
  const diagnosticsWithIndex = useMemo(
    () =>
      importDiagnostics.map((row, idx) => ({ ...row, diagnosticIndex: idx })),
    [importDiagnostics],
  );
  const diagnosticOptionCache = useMemo(() => {
    const cache = new Map();
    diagnosticsWithIndex.forEach((row) => {
      const key = `${row.feature}||${row.possibleMatches || ""}`;
      if (!cache.has(key)) {
        cache.set(
          key,
          getChildFeatureOptions(
            {
              planningGroup: row.feature,
              featureName: row.feature,
              workspaceName: row.feature,
              possibleMatches: row.possibleMatches
                ? row.possibleMatches.split(" | ")
                : [],
            },
            mappingFeatures,
          )
    ...
    return cache;
  }, [diagnosticsWithIndex, mappingFeatures]);
  const diagnosticFeatureOptions = (row) =>
    diagnosticOptionCache.get(`${row.feature}||${row.possibleMatches || ""}`) ||
    [];
  const diagnosticFeatureIds = (row) =>
    allocationFeatureIds(allocationById.get(row.allocationId) || {});
  const diagnosticNeedsReview = (row) => {
    const allocation = allocationById.get(row.allocationId) || {};
    const selectedIds = allocationFeatureIds(allocation);
    const options = diagnosticFeatureOptions(row);
    const hasChildContext = !!row.comment || isSplitPlanningGroup(row.feature);
    const fill = String(row.fillColor || "").toUpperCase();
    const expectedFill =
      (fill.includes("FFFF00") || fill.includes("92D050")) &&
      !row.expectedComplete;
    const possibleCount = String(row.possibleMatches || "")
      .split(" | ")
      .filter(Boolean).length;
    const stillAddingMultiple =
      possibleCount > 1 && selectedIds.length < possibleCount;
    return (
      row.stage === NEEDS_MAPPING_STAGE ||
      row.mappingStatus === "ambiguous" ||
      row.mappingStatus === "unmatched" ||
      stillAddingMultiple ||
      (row.comment && selectedIds.length === 0) ||
      (!row.comment &&
        hasChildContext &&
        options.length > 1 &&
        selectedIds.length === 0) ||
      expectedFill ||
      (hasChildContext && options.length === 0)
    );
  };
  const reviewDiagnostics = useMemo(
    () => diagnosticsWithIndex.filter(diagnosticNeedsReview),
    [diagnosticsWithIndex, allocationById, diagnosticOptionCache],
  );
  const visibleDiagnostics = showAllDiagnostics
    ? diagnosticsWithIndex
    : reviewDiagnostics;
  const diagnosticRowLimit = showAllDiagnostics ? 120 : 80;
  const importDiagnosticsPanel = importDiagnostics.length ? (
    <div className="panel import-diagnostics">
      <div className="panel-top">
        <div>
          <h3>Planning Import Diagnostics</h3>
          <p className="muted">Showing rows that need review or correction.</p>
        </div>
        <div className="toolbar-left">
          <button onClick={() => setShowAllDiagnostics((v) => !v)}>
            {showAllDiagnostics
              ? "Show action needed only"
      ...
      {visibleDiagnostics.length ? (
        <table className="compact-table">
          ...
          <tbody>
            {visibleDiagnostics.slice(0, diagnosticRowLimit).map((row) => {
              const options = diagnosticFeatureOptions(row);
              const selectedIds = diagnosticFeatureIds(row);
              ...
              const noContext =
                !row.comment && !isSplitPlanningGroup(row.feature);
              const isProdSupportRow = isProdSupportGroup(row.feature);
              return (
                <tr key={`${row.row}-${row.sprint}-${row.diagnosticIndex}`}>
                  <td>{row.row}</td>
                  <td>{row.stageCell}</td>
                  <td>{row.sprint}</td>
                  <td>{row.person}</td>
                  ...
                  <td>{row.stage}</td>
                  <td>{row.days}</td>
                  <td>{row.expectedComplete ? "true" : "false"}</td>
                  <td>{row.comment || "-"}</td>
                  <td>
                    {noContext ? (
                      <span className="diagnostic-na">
                        No child mapping needed
                      </span>
                    ) : options.length || isProdSupportRow ? (
                      <div className="diagnostic-mapping-cell">
                        <div className="diagnostic-selected-list">
                          {selectedFeatures.length ? (
                            selectedFeatures.map((f) => (
                              <span
                                className="diagnostic-selected-feature"
                                key={f.id}
                              >
                                {f.feature_name}
                                <button
                            ...
                            <span className="diagnostic-na">
                              No child feature selected
                            </span>
                          )}
                        </div>
                        {availableOptions.length || isProdSupportRow ? (
                          <select
                            className="diagnostic-feature-select"
                            value=""
                            onChange={(e) => {
                              handleDiagnosticFeatureSelect(
                                row,
                                e.target.value,
                              );
                              e.target.value = "";
                          ...
                          >
                            ...
                            {isProdSupportRow && (
                              <option value={CREATE_PROD_STORY_VALUE}>
                                + Create new Prod Support story
                              </option>
                            )}
                          ...
                          <small>All scoped child features selected.</small>
                        )}
                        <small>
                          Capacity days stay on this one allocation.
                        </small>
                        {row.possibleMatches && (
                          <small>Suggested: {row.possibleMatches}</small>
                        )}
                      </div>
                    ) : (
                      <span className="diagnostic-na">
                        No scoped child features
                      </span>
                    )}
                  </td>
                </tr>
        ...
        <p className="muted">
          No diagnostics rows need review. Use Show all imported rows for an
          audit view.
        </p>
      )}
      {visibleDiagnostics.length > diagnosticRowLimit && (
        <p className="muted">
          Showing first {diagnosticRowLimit} diagnostics rows.
        </p>
      )}
    </div>
  ) : null;
  const kanbanPanel = (
    <div className="panel planning-kanban-panel">
      <div className="panel-top">
        <div>
        ...
        <div className="mini-stats">
          ...
          <span>
            {customSprints.length} custom sprint
            {customSprints.length === 1 ? "" : "s"}
          </span>
        </div>
      </div>
      <div className="board-scroll">
        <div className="board compact planning-board">
          {kanbanColumns.map((col) => (
            <div className="board-col compact" key={col.stage}>
              <div className="col-head">
                <div>
                  <b>{col.label}</b>
                  <small>{col.items.length} shown</small>
                </div>
                ...
                col.items.map(
                  ({
                    ...
                    <div
                      ...
                      key={feature.id}
                      onClick={() => openPlanFeature(feature)}
                    >
                      <div className="card-top">
                        <div className="card-title">{feature.feature_name}</div>
                        <button
                      ...
                      <div className="planning-milestones">
                        {milestones.length ? (
                          milestones.map((m) => (
                            <span
                              className={m.complete ? "complete" : ""}
                              key={`${m.stage}-${m.sprint}`}
                              title={
                                m.sourceStage && m.sourceStage !== m.stage
                                  ? `Source stage: ${m.sourceStage}`
                                  : ""
                              }
                            >
                              {m.stage}: {m.sprint}
                      ...
                      <div className="card-meta">
                        ...
                        <span>Final stage: {finalStage}</span>
                      </div>
                    </div>
                  ),
                )
      ...
      <div className="modal-card small">
        <div className="panel-top">
          <div>
            ...
            <small>
              {editingAllocationRow.sprint || "Unscheduled"} /{" "}
              {editingAllocationRow.planningGroup ||
                editingAllocationRow.featureName ||
                editingAllocationRow.workspaceName ||
                "Planning item"}
            </small>
          </div>
          <button onClick={() => setEditingAllocation(null)}>Close</button>
        </div>
        <div className="form-grid">
          <label className="full">
            Planning item
          ...
          {isSplitPlanningGroup(
            ...
            <label className="full">
              Child feature
              <select
                value={editingAllocationRow.featureId || ""}
                onChange={(e) =>
                  updateAllocationFeature(
                    editingAllocationRow.id,
                    e.target.value,
                  )
                }
              >
                <option value="">Select child feature</option>
                {isProdSupportGroup(
                  editingAllocationRow.planningGroup ||
                    editingAllocationRow.featureName,
                ) && (
              ...
              {editingAllocationRow.possibleMatches?.length > 0 && (
                <small>
                  Possible matches:{" "}
                  {editingAllocationRow.possibleMatches.join(" / ")}
                </small>
              )}
              {isProdSupportGroup(
                editingAllocationRow.planningGroup ||
                ...
                <div className="prod-story-create">
                  <input
                    placeholder="Story name"
                    value={prodStoryDraft.name}
                    onChange={(e) =>
                      setProdStoryDraft({
                        ...prodStoryDraft,
                        name: e.target.value,
                  ...
                  <input
                    ...
                    value={prodStoryDraft.devOpsId}
                    onChange={(e) =>
                      setProdStoryDraft({
                        ...prodStoryDraft,
                        devOpsId: e.target.value,
                      })
                    }
                  />
                  <input
                    placeholder="Notes"
                    value={prodStoryDraft.notes}
                    onChange={(e) =>
                      setProdStoryDraft({
                        ...prodStoryDraft,
          ...
          <label>
            Owner
            <select
              value={editingAllocationRow.owner || ""}
              onChange={(e) =>
                updateAllocation(editingAllocationRow.id, {
            ...
            <small>
              {remainingFor(
                editingAllocationRow.owner,
                editingAllocationRow.sprint,
              ) == null
                ? "No capacity"
                : remainingFor(
                      editingAllocationRow.owner,
                  ...
                  ? `Over capacity by ${Math.abs(remainingFor(editingAllocationRow.owner, editingAllocationRow.sprint))} days`
                  : `Remaining in ${editingAllocationRow.sprint}: ${remainingFor(editingAllocationRow.owner, editingAllocationRow.sprint)} days`}
            </small>
          </label>
          <label>
            Stage
            <select
              value={editingAllocationRow.stage}
              onChange={(e) =>
                updateAllocation(editingAllocationRow.id, {
          ...
          <label>
            ...
            <input
              type="number"
              min="0"
              value={editingAllocationRow.days}
              onChange={(e) =>
                updateAllocation(editingAllocationRow.id, {
          ...
          {allocationFeatureIds(editingAllocationRow).length > 1 ? (
            <div className="full child-outcome-list">
              <b>Child feature outcomes</b>
              {allocationFeatureIds(editingAllocationRow).map((featureId) => {
                const feature = featureById.get(featureId);
                return (
                  <label key={featureId}>
              ...
              <small>Capacity days stay on the shared source allocation.</small>
            </div>
          ) : (
            <label className="full">
              Stage outcome
              <select
                value={
                  allocationOutcomeComplete(editingAllocationRow)
                    ? "complete"
                    : "planned"
                }
                onChange={(e) =>
                  updateAllocationOutcome(
                    editingAllocationRow,
                    e.target.value === "complete",
                  )
                }
        ...
        <div className="modal-actions">
          <button
            onClick={() => {
              deleteAllocation(editingAllocationRow.id);
              setEditingAllocation(null);
            }}
          >
            Delete
          </button>
          <button onClick={() => setEditingAllocation(null)}>Done</button>
        </div>
      </div>
    </div>
  );
  const originalSourceDays = Number(editingAllocationRow?.sourceDays ?? editingAllocationRow?.days ?? 0);
  const reconcileAllocationModal = editingAllocationRow && (
    <div className="modal"><div className="modal-card reconcile-modal">
      <div className="panel-top"><div><h2>Reconcile Allocation</h2><small>{editingAllocationRow.sprint || "Unscheduled"} / {allocationWorkspace(editingAllocationRow)}</small></div><button onClick={() => setEditingAllocation(null)}>Close</button></div>
      <div className="source-context">
        <div><b>Source row / cell</b><span>{editingAllocationRow.sourceRow || editingAllocationRow.stageCell || "Imported allocation"}</span></div>
        <div><b>Sprint</b><span>{editingAllocationRow.sprint || "Unscheduled"}</span></div>
        <div><b>Workspace / group</b><span>{allocationWorkspace(editingAllocationRow)}</span></div>
      ...
      <div className="modal-card">
          ...
          <div>
            ...
            <small>
              ...
              {STATUS_LABEL[selectedPlanFeature.status] ||
                "Standalone planning item"}
            </small>
            <small>
              Planned Through: {entityThrough(selectedPlanFeature)} / Next
              recommended: {entityNext(selectedPlanFeature)}
            </small>
            <small>
        ...
        <div className="final-stage-control">
          <label>
            ...
            <select
              value={featureFinalStage(selectedPlanFeature)}
              onChange={(e) =>
                updateFeatureFinalStage(selectedPlanFeature.id, e.target.value)
              }
            >
              {PLAN_STAGES.filter((stage) => stage !== "Requirement").map(
                (stage) => (
          ...
          <small>
            When this stage is Expected complete, the card moves to Planning
            Complete. Board View items default to BA Sign-off; TSNSW Adult
            Community Education defaults to UAT.
          </small>
        </div>
        <h3>Existing planning allocations</h3>
        <table>
          <thead>
            <tr>
              <th>Sprint</th>
          ...
          <tbody>
            {selectedAllocations.length ? (
                ...
                <tr key={a.id}>
                  ...
                  <td>
                    <select
                      value={a.stage}
                      onChange={(e) =>
                        updateAllocation(a.id, { stage: e.target.value })
                      }
                    >
                      {STAGE_OPTIONS.map((s) => (
                        <option key={s}>{s}</option>
                  ...
                  <td>
                    <select
                      value={a.owner}
                      onChange={(e) =>
                        updateAllocation(a.id, { owner: e.target.value })
                      }
                    >
                      {ownerOptions.map((o) => (
                        <option key={o}>{o}</option>
                  ...
                  <td>
                    <select
                      value={
                        allocationOutcomeComplete(a) ? "complete" : "planned"
                      }
                      onChange={(e) =>
                        updateAllocationOutcome(
                          a,
              ...
              <tr>
                <td colSpan="7">No planning allocations yet.</td>
              </tr>
            )}
          </tbody>
        </table>
        <>
            <h3>Add planning allocation</h3>
            <div className="form-grid">
              <label>
                Stage
                <select
                  value={quickPlan.stage}
                  onChange={(e) =>
                    setQuickPlan({ ...quickPlan, stage: e.target.value })
                  }
                >
                  {STAGE_OPTIONS.map((s) => (
                    <option key={s}>{s}</option>
              ...
              <label>
                Owner
                <select
                  value={quickPlan.owner}
                  onChange={(e) =>
                    setQuickPlan({ ...quickPlan, owner: e.target.value })
                  }
                >
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setQuickPlan({ ...quickPlan, days: e.target.value })
                  }
                />
              </label>
              <label>
                Stage outcome
                <select
                  value={quickPlan.isStageComplete ? "complete" : "planned"}
                  onChange={(e) =>
                    setQuickPlan({
                      ...quickPlan,
                      isStageComplete: e.target.value === "complete",
                    })
                  }
                >
                  <option value="planned">Planned</option>
              ...
              <div className="capacity-chip-section">
                {quickPlan.owner && selectedRemaining != null && (
                  <p className={"selected-capacity " + selectedCapacityTone}>
                    {quickPlan.owner}: {selectedRemaining} day
                    {Math.abs(selectedRemaining) === 1 ? "" : "s"} remaining in{" "}
                    {quickPlan.sprint}
                  </p>
                )}
                ...
                {selectedRemaining != null &&
                  Number(quickPlan.days || 0) > selectedRemaining && (
                    <p className="capacity-warning">
                      Warning: this exceeds remaining capacity by{" "}
                      {Number(quickPlan.days || 0) - selectedRemaining} day(s).
                    </p>
                  )}
              </div>
            </div>
            <div className="modal-actions">
              <button onClick={() => setSelectedPlanFeature(null)}>
                Close
              </button>
              <button
                disabled={!quickPlan.owner || Number(quickPlan.days || 0) < 0}
                onClick={() => saveRecommendedAllocation(selectedPlanFeature)}
              >
                Add Allocation &amp; Keep Planning
              </button>
            </div>
          </>
      </div>
  ...
  const capacityTools = (
    <div className="capacity-tools">
      <div className="panel sprint-planner-panel compact-capacity-panel">
        ...
        <table className="compact-table capacity-table">
          <thead>
            <tr>
              <th>Owner</th>
              <th>Base</th>
              ...
              <tr key={r.owner}>
                ...
                <td>
                  <input
                    ...
                    value={r.base ?? ""}
                    placeholder="-"
                    onChange={(e) => updateCapacity(r.owner, e.target.value)}
                  />
                </td>
                <td>{r.off || ""}</td>
                <td>{r.available == null ? "-" : r.available}</td>
                <td>{r.planned}</td>
                <td className={r.remaining < 0 ? "capacity-negative" : ""}>
      ...
      <div className="panel days-off-panel">
        <div className="panel-top compact-panel-top">
          <div>
            <h3>Days Off / Capacity</h3>
            <p className="muted">Days off deduct from available capacity.</p>
          </div>
        </div>
        <div className="days-off-form">
            ...
            <input
              ...
              onChange={(e) =>
                setDayOffForm({
                  ...
                  endDate: dayOffForm.endDate || e.target.value,
                })
              }
            />
          </label>
          <label>
            ...
            <input
              ...
              onChange={(e) =>
                setDayOffForm({ ...dayOffForm, endDate: e.target.value })
              }
            />
          </label>
          <label>
            ...
            <input
              ...
              onChange={(e) =>
                setDayOffForm({ ...dayOffForm, note: e.target.value })
              }
            />
          </label>
            ...
            {effectiveSprintDates.slice(0, 7).map((row) => (
              <div key={row.sprint} className="sprint-date-row">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateSprintDates(row.sprint, { startDate: e.target.value })
                  }
                />
                <input
                  type="date"
                  ...
                  onChange={(e) =>
                    updateSprintDates(row.sprint, { endDate: e.target.value })
                  }
                />
              </div>
            ))}
        ...
        <table className="compact-table days-off-table">
          <thead>
            <tr>
              <th>Person</th>
              <th>Sprint</th>
          ...
          <tbody>
            {visibleDaysOff.length ? (
              visibleDaysOff.map((row) => (
                <tr key={row.id}>
                  <td>{row.owner}</td>
                  <td>
                    <b>
                  ...
                  <td>
                    ...
                    {row.endDate && row.endDate !== row.startDate
                      ? ` to ${fmtDate(row.endDate)}`
                      : ""}
                  </td>
                  <td>
                    {Number(
                      row.days ||
                        workingDaysBetween(row.startDate, row.endDate) ||
                        0,
                    )}
                  </td>
                  <td>{row.note || "-"}</td>
              ...
              <tr>
                <td colSpan="6">No days off for this sprint.</td>
              </tr>
            )}
          </tbody>
        </table>
    ...
    <div className="dashboard">
      <div className="dash-head">
        <div>
          ...
          <h1>Stage-by-stage sprint planning</h1>
        </div>
        <div className="toolbar-left">
          <button
            onClick={() => document.getElementById("planning-xlsx").click()}
          >
            Import 26Q1 Planning Excel
          </button>
          <input
            id="planning-xlsx"
            type="file"
          ...
          >
            ...
            {allFeatureWorkspaces.map((w) => (
              <option key={w}>{w}</option>
            ))}
          </select>
        </div>
      </div>
      <div className="panel planning-controls">
        <div className="toolbar-left">
          <input
            value={newSprint}
...
function App() {
  ...
  const [theme, setTheme] = useState(
    () => localStorage.getItem("feature-tracker-theme") || "dark",
  );
  ...
  useEffect(() => {
    try {
      ...
      if (saved?.features) {
        const normalisedFeatures = saved.features.map((feature) => ({ ...feature, workspace: normaliseWorkspaceName(feature.workspace) }));
        setFeatures(normalisedFeatures);
        setMilestones(
          buildWorkspaceMilestones(normalisedFeatures, saved.milestones || {}),
        );
        setAllocations((saved.allocations || []).flatMap(expandSharedAllocation));
        setCapacities(saved.capacities || []);
        setDaysOff(saved.daysOff || []);
  ...
  useEffect(() => {
    localStorage.setItem("feature-tracker-theme", theme);
  }, [theme]);
  const workspaces = useMemo(
    () => [
      "ALL",
      ...Array.from(
        new Set(features.map((f) => f.workspace).filter(Boolean)),
      ).sort(),
    ],
    [features],
  );
  const owners = useMemo(
    () => [
      "ALL",
      ...Array.from(
        new Set(features.flatMap((f) => splitOwners(f.owner))),
      ).sort(),
    ],
    [features],
  );
  function importFile(e) {
    ...
    reader.onload = () => {
      ...
      setMilestones(buildWorkspaceMilestones(cleaned, inputMilestones));
    };
    reader.readAsText(file);
    e.target.value = "";
  }
  function exportFile() {
    ...
    const rows = features.map((f) => [
      ...
      milestones[f.workspace]?.["UAT Internal"] || "",
    ]);
    const csv = [
      headers.join(","),
      ...rows.map((r) => r.map(csvEscape).join(",")),
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "feature-tracker-export.csv";
    a.click();
    URL.revokeObjectURL(url);
  }
    ...
    <div className={`app ${theme}`}>
      <div className="toolbar">
        <div className="toolbar-left">
          ...
          <button
            className={mode === "sprintReview" ? "active" : ""}
            onClick={() => setMode("sprintReview")}
          >
            Sprint Review
          </button>
          <button
        ...
        <div className="mini-stats">
          <span>
            Total <b>{features.length}</b>
          </span>
          <span>
            Done <b>{features.filter((f) => f.status === "uat_done").length}</b>
          </span>
        </div>
      </div>
      {mode === "executive" && (
      ...
      {mode === "delivery" && (
        <DeliveryPlan
          ...
          customSprints={customSprints}
          setCustomSprints={setCustomSprints}
          owners={owners}
          daysOff={daysOff}
          setDaysOff={setDaysOff}
          sprintDates={sprintDates}
          setSprintDates={setSprintDates}
          prodSupportStories={prodSupportStories}

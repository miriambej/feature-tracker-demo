...
const TODAY = new Date(2026, 3, 18); // 18 Apr 2026
const CURRENT_SPRINT_END = new Date(2026, 3, 21);
const NEXT_SPRINT_END = new Date(2026, 4, 5);
...
function ExecutiveDashboard({ features, milestones, setMilestones, workspaces, workspaceFilter, setWorkspaceFilter, selectedWorkspace, setSelectedWorkspace, onEditFeature }) {
  ...
  return <div className="dashboard">
    ...
    <div className="panel"><h3>Milestone roadmap</h3><div className="timeline-head" style={{ gridTemplateColumns: `260px repeat(${months.length}, minmax(68px,1fr)) 150px` }}><span>Workspace</span>{months.map((m,i)=><span key={i} className={i===todayIdx?'today-month':''}>{monthLabel(m)}</span>)}<span>Health</span></div>{summaries.map(s => <div className="timeline-row" key={s.workspace} onClick={() => setSelectedWorkspace(s.workspace)}><div><b>{s.workspace}</b><small>{s.features.length} features · {s.blockers.length} blockers</small></div><div className="timeline-track" style={{ gridTemplateColumns: `repeat(${months.length}, minmax(68px,1fr))` }}>{months.map((m,i)=><div key={i} className={i===todayIdx?'today-cell':''}/>) }{todayIdx>=0&&<div className="today-line" style={{left:`calc(${((todayIdx+.5)/months.length)*100}% - 1px)`}}/>}{s.milestones.map(m=>{const idx=monthIndex(parseDate(s.dates[m.key]),months);return idx<0?null:<div key={m.key} className={`dot ${m.health.tone}`} style={{left:`calc(${((idx+.5)/months.length)*100}% - 12px)`}} title={`${m.label} ${fmtDate(s.dates[m.key])}`}>{m.short}</div>})}</div><div className="row-actions"><StatusPill tone={s.overallTone} label={s.overallLabel}/><button onClick={(e)=>{e.stopPropagation();setEditWs(s.workspace)}}>Edit dates</button></div></div>)}</div>
    <div className="two-col"><div className="panel"><h3>Workspace milestone summary</h3><table><thead><tr><th>Workspace</th><th>Build</th><th>SIT</th><th>UAT Internal</th><th>Overall</th><th>Blocking</th></tr></thead><tbody>{summaries.map(s=><tr key={s.workspace} onClick={()=>setSelectedWorkspace(s.workspace)}><td>{s.workspace}</td>{s.milestones.map(m=><td key={m.key}><StatusPill tone={m.health.tone} label={m.health.label}/><small>{fmtDate(s.dates[m.key])}</small></td>)}<td><StatusPill tone={s.overallTone} label={s.overallLabel}/></td><td>{s.blockers.length}</td></tr>)}</tbody></table></div><WorkspaceDetail summary={selected} onClose={()=>setSelectedWorkspace(null)} onEdit={onEditFeature}/></div>
...
function OverviewDashboard({ features, milestones, workspaces, workspaceFilter, setWorkspaceFilter }) {
  ...
  return <div className="dashboard"><div className="dash-head"><div><div className="eyebrow">Overview Dashboard</div><h1>Stage health and next sprint focus</h1></div><select value={workspaceFilter} onChange={e=>setWorkspaceFilter(e.target.value)}>{workspaces.map(w=><option key={w}>{w}</option>)}</select></div><div className="summary-grid"><SummaryCard label="Total Features" value={total} hint="Current view"/><SummaryCard label="In Flight" value={filtered.filter(f=>!['initial','uat_done'].includes(f.status)).length} hint="Not initial or done"/><SummaryCard label="Ready Next" value={nextActions.length} hint="Ready for next stage"/><SummaryCard label="Total Users" value={totalUsers.toLocaleString()} hint="Usage impact"/><SummaryCard label="Done" value={grouped.uat_done||0} hint="UAT done"/><SummaryCard label="Progress" value={`${total?Math.round(filtered.reduce((s,f)=>s+score(f.status),0)/total):0}%`} hint="Weighted maturity"/></div><div className="two-col"><div className="panel"><h3>Next sprint priorities (ending {NEXT_SPRINT_END.toLocaleDateString(undefined,{day:'numeric',month:'short'})})</h3>{summaries.length?summaries.map(s=><div className="priority-block" key={s.workspace}><b>{s.workspace}</b><small>{s.dueNextBlockers.length} blockers · {s.dueNextBlockers.filter(f=>priority(f.user_count)==='Critical').length} critical</small>{s.dueNextBlockers.slice(0,5).map(f=><div key={f.id} className={`queue-card ${priorityClass(f.user_count)}`}><div><b>{f.feature_name}</b><small>{f.workspace} · {STATUS_LABEL[f.status]}</small></div><span>{priority(f.user_count)}</span></div>)}</div>):<p className="muted">No workspace milestones due next sprint.</p>}</div><div className="panel"><h3>Stage breakdown</h3>{STATUSES.map(([k,l])=><div className="bar-row" key={k}><span>{l}</span><b>{grouped[k]||0}</b><div><i style={{width:`${total?(grouped[k]||0)/total*100:0}%`}}/></div></div>)}</div></div><div className="panel"><h3>Top next actions</h3>{nextActions.length?nextActions.map(f=><div className={`queue-card ${priorityClass(f.user_count)}`} key={f.id}><div><b>{f.feature_name}</b><small>{f.workspace} · {STATUS_LABEL[f.status]}</small></div><span>{f.user_count||0} users</span></div>):<p className="muted">No features currently ready for the next action.</p>}</div></div>;
}

function App(){
 const [features,setFeatures]=useState([]); const [milestones,setMilestones]=useState({}); const [mode,setMode]=useState('executive'); const [workspaceFilter,setWorkspaceFilter]=useState('ALL'); const [ownerFilter,setOwnerFilter]=useState('ALL'); const [selectedWorkspace,setSelectedWorkspace]=useState(null); const [editing,setEditing]=useState(null); const [loaded,setLoaded]=useState(false);

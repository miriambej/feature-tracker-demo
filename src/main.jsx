...
function DeliveryPlan({ features, allocations, setAllocations, capacities, setCapacities, owners }) {
  ...
  const ownerOptions=useMemo(()=>Array.from(new Set([...owners.filter(o=>o!=='ALL'), ...capacities.map(c=>c.owner), ...allocations.map(a=>a.owner)].filter(Boolean))).sort(),[owners,capacities,allocations]);
  ...
  async function importPlanningExcel(e){const file=e.target.files?.[0]; if(!file)return; try{const XLSX=await import(/* @vite-ignore */ 'https://cdn.sheetjs.com/xlsx-0.20.3/package/xlsx.mjs'); const wb=XLSX.read(await file.arrayBuffer(),{type:'array'}); const sheet=wb.Sheets['26Q1 Planner']; if(!sheet){setImportMessage('Sheet "26Q1 Planner" was not found.');return;} const rows=XLSX.utils.sheet_to_json(sheet,{header:1,blankrows:false}); const sprintCols=[]; rows.slice(0,15).forEach(row=>row.forEach((cell,col)=>{const label=String(cell||'').trim(); if(col>=17&&/^S[1-7]$/i.test(label)) sprintCols.push({label:label.toUpperCase(),col});})); if(!sprintCols.length) for(let i=0;i<7;i++) sprintCols.push({label:`S${i+1}`,col:17+i*3}); const imported=[]; let currentFeature=''; rows.forEach(row=>{const featureCell=String(row[2]||'').trim(); if(featureCell) currentFeature=featureCell; if(!currentFeature) return; sprintCols.forEach(({label,col})=>{const owner=String(row[col]||'').trim(); const stage=String(row[col+1]||'').trim(); const days=Number(row[col+2]||0); if((owner||stage)&&days){const match=features.find(f=>f.feature_name.trim().toLowerCase()===currentFeature.toLowerCase()); imported.push({id:id(),featureId:match?.id||'',featureName:currentFeature,workspace:match?.workspace||'',sprint:`26Q1${label}`,owner,stage:stage||'Build',days});}});}); setAllocations(prev=>[...imported,...prev]); setImportMessage(`Planning allocations imported. Person-level capacity not found in this file. Imported ${imported.length} allocation(s).`);}catch(err){setImportMessage(`Excel import failed: ${err.message}`);} finally {e.target.value='';}}

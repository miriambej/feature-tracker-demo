...
function nextPlanStage(featureId, allocations) {
  const through = plannedThrough(featureId, allocations);
...
function DeliveryPlan({ features, allocations, setAllocations, capacities, setCapacities, owners }) {
  ...
  const queueItems=filteredFeatures.map(f=>{const through=plannedThrough(f.id,allocations); const next=nextPlanStage(f.id,allocations); const nextAllocation=allocations.find(a=>a.featureId===f.id&&a.stage===next); return{feature:f,through,next,nextAllocation};}).filter(x=>x.next!=='Planning Complete'&&(ownerFilter==='ALL'||allocations.some(a=>a.featureId===x.feature.id&&a.owner===ownerFilter))&&(sprint==='ALL'||!allocations.some(a=>a.featureId===x.feature.id)||allocations.some(a=>a.featureId===x.feature.id&&a.sprint===sprint)));

...
function DeliveryPlan({ features, allocations, setAllocations, capacities, setCapacities, owners }) {
  ...
  async function importPlanningExcel(e){const file=e.target.files?.[0]; if(!file)return; try{const XLSX=await import(/* @vite-ignore */ 'https://cdn.sheetjs.com/xlsx-0.20.3/package/xlsx.mjs'); const wb=XLSX.read(await file.arrayBuffer(),{type:'array'}); const sheet=wb.Sheets['26Q1 Planner']; if(!sheet){setImportMessage('Sheet "26Q1 Planner" was not found.');return;} const rows=XLSX.utils.sheet_to_json(sheet,{header:1,blankrows:false}); const sprintCols=Array.from({length:7},(_,i)=>({label:`S${i+1}`,col:17+i*3})); const imported=[]; let currentFeature=''; rows.slice(12,184).forEach(row=>{const featureCell=String(row[2]||'').trim(); if(featureCell) currentFeature=featureCell; if(!currentFeature) return; sprintCols.forEach(({label,col})=>{const owner=String(row[col]||'').trim(); const stage=String(row[col+1]||'').trim(); const days=Number(row[col+2]||0); if(owner&&days&&!isRolePlaceholder(owner)){const match=features.find(f=>f.feature_name.trim().toLowerCase()===currentFeature.toLowerCase()); imported.push({id:id(),featureId:match?.id||'',featureName:currentFeature,workspace:match?.workspace||'',sprint:`26Q1${label}`,owner,stage:stage||'Build',days});}});}); const importedCaps=[]; rows.slice(188,199).forEach(row=>{sprintCols.forEach(({label,col})=>{const owner=String(row[col]||'').trim(); const availableDays=Number(row[col+1]||0); if(owner&&Number.isFinite(availableDays)&&availableDays>0&&!isRolePlaceholder(owner)){importedCaps.push({sprint:`26Q1${label}`,owner,availableDays});}});}); setAllocations(imported); setCapacities(importedCaps); setImportMessage(importedCaps.length?`Planning allocations and person-level capacity imported. Imported ${imported.length} allocation(s) and ${importedCaps.length} capacity row(s).`:`Planning allocations imported. Person-level capacity not found in this file. Imported ${imported.length} allocation(s).`);}catch(err){setImportMessage(`Excel import failed: ${err.message}`);} finally {e.target.value='';}}

...
function DeliveryPlan({ features, allocations, setAllocations, capacities, setCapacities, owners }) {
  ...
  const [form,setForm]=useState({featureId:'',stage:'Build',sprint:sprintOptions[0] || '26Q1S1',owner:ownerOptions[0]||'',days:1});
  ...
  const [quickPlan,setQuickPlan]=useState({sprint:sprintOptions[0] || '26Q1S1',owner:'',days:1});
  ...
  const kanbanBase=(planWorkspaceFilter==='ALL'?features:features.filter(f=>f.workspace===planWorkspaceFilter)).filter(f=>(ownerFilter==='ALL'||splitOwners(f.owner).includes(ownerFilter)||allocations.some(a=>a.featureId===f.id&&a.owner===ownerFilter))&&(kanbanSprint==='ALL'||allocations.some(a=>a.featureId===f.id&&a.sprint===kanbanSprint)));
  const kanbanItems=kanbanBase.map(feature=>{const featureAllocations=allocations.filter(a=>a.featureId===feature.id); const through=effectivePlannedThrough(feature,allocations); const next=nextPlanStage(feature.id,allocations,feature); const sprintAllocations=kanbanSprint==='ALL'?featureAllocations:featureAllocations.filter(a=>a.sprint===kanbanSprint); return{feature,through,next,latest:latestPlannedSprint(feature.id,allocations),sprintStage:sprintAllocations.map(a=>a.stage).join(', ')||'—'};}).sort((a,b)=>priorityRank(a.feature.user_count)-priorityRank(b.feature.user_count)||Number(b.feature.user_count||0)-Number(a.feature.user_count||0));
  ...
  const selectedAllocations=selectedPlanFeature?allocations.filter(a=>a.featureId===selectedPlanFeature.id).sort((a,b)=>String(a.sprint).localeCompare(String(b.sprint))||PLAN_STAGES.indexOf(a.stage)-PLAN_STAGES.indexOf(b.stage)):[];
  return <div className="dashboard"><div className="dash-head"><div><div className="eyebrow">Delivery Plan</div><h1>Stage-by-stage sprint planning</h1></div><div className="toolbar-left"><button onClick={()=>document.getElementById('planning-xlsx').click()}>Import 26Q1 Planning Excel</button><input id="planning-xlsx" type="file" accept=".xlsx,.xls" hidden onChange={importPlanningExcel}/><select value={sprint} onChange={e=>setSprint(e.target.value)}>{sprintOptions.map(s=><option key={s}>{s}</option>)}</select><select value={ownerFilter} onChange={e=>setOwnerFilter(e.target.value)}><option value="ALL">All Owners</option>{ownerOptions.map(o=><option key={o}>{o}</option>)}</select><select value={planWorkspaceFilter} onChange={e=>setPlanWorkspaceFilter(e.target.value)}><option value="ALL">All Workspaces</option>{Array.from(new Set(features.map(f=>f.workspace).filter(Boolean))).sort().map(w=><option key={w}>{w}</option>)}</select></div></div>{importMessage&&<div className="panel import-message">{importMessage}</div>}<div className="two-col"><div className="panel"><h3>Sprint Planner</h3><table><thead><tr><th>Owner</th><th>Available Days</th><th>Planned Days</th><th>Remaining Days</th></tr></thead><tbody>{capacityRows.map(r=><tr key={r.owner}><td>{r.owner}</td><td><input type="number" value={r.available??''} placeholder="—" onChange={e=>updateCapacity(r.owner,e.target.value)}/></td><td>{r.planned}</td><td className={r.remaining<0?'capacity-negative':''}>{r.remaining==null?'—':r.remaining}</td></tr>)}</tbody></table></div><div className="panel"><h3>Add Allocation</h3><div className="form-grid"><label>Feature<select value={form.featureId} onChange={e=>setForm({...form,featureId:e.target.value})}>{features.map(f=><option key={f.id} value={f.id}>{f.feature_name}</option>)}</select></label><label>Stage<select value={form.stage} onChange={e=>setForm({...form,stage:e.target.value})}>{PLAN_STAGES.map(s=><option key={s}>{s}</option>)}</select></label><label>Sprint<select value={form.sprint} onChange={e=>setForm({...form,sprint:e.target.value})}>{sprintOptions.map(s=><option key={s}>{s}</option>)}</select></label><label>Owner<select value={form.owner} onChange={e=>setForm({...form,owner:e.target.value})}>{ownerOptions.map(o=><option key={o}>{o}</option>)}</select></label><label>Days<input type="number" min="0" value={form.days} onChange={e=>setForm({...form,days:e.target.value})}/></label></div>{warning&&<p className="capacity-warning">{warning}</p>}<div className="modal-actions"><button onClick={saveAllocation}>Save</button></div></div></div><div className="panel"><div className="panel-top"><div><h3>Planning Kanban</h3><p className="muted">Uses actual delivery status plus planning allocations to recommend the next stage.</p></div><div className="toolbar-left"><select value={kanbanSprint} onChange={e=>setKanbanSprint(e.target.value)}><option value="ALL">All Sprints</option>{sprintOptions.map(s=><option key={s}>{s}</option>)}</select><select value={ownerFilter} onChange={e=>setOwnerFilter(e.target.value)}><option value="ALL">All Owners</option>{ownerOptions.map(o=><option key={o}>{o}</option>)}</select><select value={planWorkspaceFilter} onChange={e=>setPlanWorkspaceFilter(e.target.value)}><option value="ALL">All Workspaces</option>{Array.from(new Set(features.map(f=>f.workspace).filter(Boolean))).sort().map(w=><option key={w}>{w}</option>)}</select></div></div><div className="board-scroll"><div className="board compact">{kanbanColumns.map(col=><div className="board-col compact" key={col.stage}><div className="col-head"><div><b>{col.label}</b><small>{col.items.length} shown</small></div><span>{col.items.length}</span></div>{col.items.length?col.items.map(({feature,through,next,latest,sprintStage})=><div className={`feature-card compact ${priorityClass(feature.user_count)}`} key={feature.id} onClick={()=>{setSelectedPlanFeature(feature);setQuickPlan(q=>({...q,sprint:q.sprint==='ALL'?sprintOptions[0]:q.sprint,owner:q.owner||ownerOptions[0]||''}));}}><div className="card-top"><div className="card-title">{feature.feature_name}</div><span className="user-pill">{Number(feature.user_count||0).toLocaleString()}</span></div><div className="card-workspace">{feature.workspace}</div><div className="card-meta"><span>Current: {STATUS_LABEL[feature.status]}</span><span>{priority(feature.user_count)}</span></div><div className="card-meta"><span>Through: {through}</span><span>Next: {next}</span></div><div className="card-meta"><span>Latest: {latest}</span><span>{kanbanSprint==='ALL'?'':`Sprint: ${sprintStage}`}</span></div></div>):<div className="empty">Nothing here yet.</div>}</div>)}</div></div>{selectedPlanFeature&&<div className="detail-section"><div className="panel-top"><div><h3>{selectedPlanFeature.feature_name}</h3><small>{selectedPlanFeature.workspace} · Current: {STATUS_LABEL[selectedPlanFeature.status]}</small><small>Planned Through: {effectivePlannedThrough(selectedPlanFeature,allocations)} · Next recommended: {nextPlanStage(selectedPlanFeature.id,allocations,selectedPlanFeature)}</small></div><button onClick={()=>setSelectedPlanFeature(null)}>Close</button></div><table><thead><tr><th>Sprint</th><th>Stage</th><th>Owner</th><th>Days</th></tr></thead><tbody>{selectedAllocations.length?selectedAllocations.map(a=><tr key={a.id}><td>{a.sprint}</td><td>{a.stage}</td><td>{a.owner}</td><td>{a.days}</td></tr>):<tr><td colSpan="4">No planning allocations yet.</td></tr>}</tbody></table>{nextPlanStage(selectedPlanFeature.id,allocations,selectedPlanFeature)!=='Planning Complete'&&<div className="form-grid"><label>Next stage<input value={nextPlanStage(selectedPlanFeature.id,allocations,selectedPlanFeature)} readOnly/></label><label>Sprint<select value={quickPlan.sprint} onChange={e=>setQuickPlan({...quickPlan,sprint:e.target.value})}>{sprintOptions.map(s=><option key={s}>{s}</option>)}</select></label><label>Owner<select value={quickPlan.owner} onChange={e=>setQuickPlan({...quickPlan,owner:e.target.value})}>{ownerOptions.map(o=><option key={o}>{o}</option>)}</select></label><label>Days<input type="number" min="0" value={quickPlan.days} onChange={e=>setQuickPlan({...quickPlan,days:e.target.value})}/></label><div className="modal-actions"><button onClick={()=>saveRecommendedAllocation(selectedPlanFeature)}>Add next allocation</button></div></div>}</div>}</div><div className="two-col"><div className="panel"><h3>Planning Allocations by Sprint</h3>{allocationSprints.length?allocationSprints.map(sp=><div className="detail-section" key={sp}><h4>{sp}</h4>{visibleAllocations.filter(a=>a.sprint===sp).map(a=><div className="queue-card" key={a.id}><div><b>{a.planningGroup || a.featureName || a.workspaceName || a.workspace || 'No planning group'}</b>{a.planningGroup&&a.featureName&&<small>Allocated Feature: {a.featureName}</small>}<small>{a.stage} · {a.owner}</small>{isSplitPlanningGroup(a.planningGroup||a.featureName)&&<label>Allocated Feature<select value={a.featureId||''} onChange={e=>updateAllocationFeature(a.id,e.target.value)}><option value="">Select child feature</option>{getChildFeatureOptions(a,features).map(f=><option key={f.id} value={f.id}>{f.feature_name}</option>)}</select></label>}</div><div className="detail-card-actions"><input type="number" min="0" value={a.days} onChange={e=>updateAllocationDays(a.id,e.target.value)}/><span>{a.sprint}</span></div></div>)}</div>):<p className="muted">No planning allocations yet.</p>}</div><div className="panel"><h3>Planned Days by Owner / Sprint</h3><table><thead><tr><th>Sprint</th><th>Owner</th><th>Planned Days</th><th>Remaining</th></tr></thead><tbody>{Object.values(ownerSprintTotals).map(r=>{const cap=capacities.find(c=>c.sprint===r.sprint&&c.owner===r.owner);return <tr key={`${r.sprint}-${r.owner}`}><td>{r.sprint}</td><td>{r.owner}</td><td>{r.planned}</td><td>{cap?Number(cap.availableDays||0)-r.planned:'—'}</td></tr>})}</tbody></table></div></div><div className="panel"><h3>Workspace Planning Summary</h3><table><thead><tr><th>Workspace</th><th>Total</th><th>Planned</th><th>Not planned</th><th>Build</th><th>SIT</th><th>Deploy</th><th>BA Sign Off</th><th>UAT Internal</th><th>Complete</th></tr></thead><tbody>{workspacePlanSummary.map(w=><tr key={w.workspace}><td>{w.workspace}</td><td>{w.total}</td><td>{w.planned}</td><td>{w.notPlanned}</td><td>{w.build}</td><td>{w.sit}</td><td>{w.deploy}</td><td>{w.ba}</td><td>{w.uat}</td><td>{w.complete}</td></tr>)}</tbody></table></div><div className="panel"><h3>Planning Queue</h3>{queueGroups.map(group=><div className="detail-section" key={group.workspace}><div className="panel-top"><div><b>{group.workspace}</b><small>{group.total} total features · {group.notPlanned} not planned</small><small>{PLAN_STAGES.map(stage=>`${stage}: ${group.counts[stage]||0}`).join(' · ')}</small></div></div><div className="queue-grid delivery-queue">{group.items.map(({feature,through,next,nextAllocation})=><div className={`queue-card ${priorityClass(feature.user_count)}`} key={feature.id}><div><b>{feature.feature_name}</b><small>{STATUS_LABEL[feature.status]}</small><small>Planned Through: {through}</small><small>Next to Plan: {next}</small>{nextAllocation&&<small>Next allocation: {nextAllocation.owner} · {nextAllocation.sprint}</small>}</div><span className="user-pill">{feature.user_count||0} users</span></div>)}</div></div>)}</div></div>;
}

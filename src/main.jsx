...
const PLAN_STAGES = ['Build', 'SIT', 'Deploy', 'BA Sign Off', 'UAT Internal'];
...
function actualPlannedThrough(status) {
  ...
  if (['uat_in_progress', 'uat_done'].includes(status)) return 'UAT Internal';
  return 'Not planned';
}
...
function DeliveryPlan({ features, allocations, setAllocations, capacities, setCapacities, owners }) {
  const sprintOptions = useMemo(() => {
    const fromData = Array.from(new Set([...capacities.map(c => c.sprint), ...allocations.map(a => a.sprint)].filter(Boolean)));
  ...
  const ownerOptions=useMemo(()=>Array.from(new Set([...owners.filter(o=>o!=='ALL'), ...capacities.map(c=>c.owner), ...allocations.map(a=>a.owner)].filter(o=>o&&!isRolePlaceholder(o)))).sort(),[owners,capacities,allocations]);
  const [ownerFilter,setOwnerFilter]=useState('ALL');
  const [planWorkspaceFilter,setPlanWorkspaceFilter]=useState('ALL');
  const [form,setForm]=useState({featureId:'',stage:'Build',sprint:sprintOptions[0] || '26Q1S1',owner:ownerOptions[0]||'',days:1,isStageComplete:false});
  const [warning,setWarning]=useState('');
  const [importMessage,setImportMessage]=useState('');
  const [kanbanSprint,setKanbanSprint]=useState('ALL');
  const [selectedPlanFeature,setSelectedPlanFeature]=useState(null);
  const [quickPlan,setQuickPlan]=useState({stage:'Build',sprint:sprintOptions[0] || '26Q1S1',owner:'',days:1,isStageComplete:false});
  useEffect(()=>{setForm(f=>({...f,sprint:f.sprint||sprint,owner:f.owner||ownerOptions[0]||'',featureId:f.featureId||features[0]?.id||''}));},[sprint,ownerOptions,features]);
  ...
  const entityNext=feature=>{const idx=PLAN_STAGES.indexOf(entityThrough(feature));return idx>=PLAN_STAGES.length-1?'Planning Complete':PLAN_STAGES[idx+1];};
  ...
  const visibleAllocations=ownerFilter==='ALL'?allocations:allocations.filter(a=>a.owner===ownerFilter);
  const capacityRows=visibleOwners.map(owner=>{const cap=capacities.find(c=>c.sprint===sprint&&c.owner===owner);const available=cap?Number(cap.availableDays||0):null;const planned=allocations.filter(a=>a.sprint===sprint&&a.owner===owner).reduce((n,a)=>n+Number(a.days||0),0);return{owner,available,planned,remaining:available==null?null:available-planned};});
  const allocationSprints=Array.from(new Set(visibleAllocations.map(a=>a.sprint).filter(Boolean))).sort();
  const ownerSprintTotals=visibleAllocations.reduce((map,a)=>{const key=`${a.sprint}||${a.owner}`; if(!map[key]) map[key]={sprint:a.sprint,owner:a.owner,planned:0}; map[key].planned+=Number(a.days||0); return map;},{});
  function remainingFor(owner,sprintId){const cap=capacities.find(c=>c.sprint===sprintId&&c.owner===owner); if(!cap)return null; const planned=allocations.filter(a=>a.sprint===sprintId&&a.owner===owner).reduce((n,a)=>n+Number(a.days||0),0); return Number(cap.availableDays||0)-planned;}
  function updateCapacity(owner,availableDays){setCapacities(prev=>{const rest=prev.filter(c=>!(c.sprint===sprint&&c.owner===owner));return[...rest,{sprint,owner,availableDays:Number(availableDays||0)}];});}
  function updateAllocationDays(allocationId,days){setAllocations(prev=>prev.map(a=>a.id===allocationId?{...a,days:Number(days||0)}:a));}
  function updateAllocationComplete(allocationId,isStageComplete){setAllocations(prev=>prev.map(a=>a.id===allocationId?{...a,isStageComplete}:a));}
  function updateAllocationFeature(allocationId,featureId){const feature=features.find(f=>f.id===featureId); if(!feature)return; setAllocations(prev=>prev.map(a=>a.id===allocationId?{...a,featureId:feature.id,actualFeatureName:feature.feature_name,featureName:feature.feature_name,workspace:feature.workspace}:a));}
  function saveAllocation(){const feature=features.find(f=>f.id===form.featureId); if(!feature) return; const days=Number(form.days||0); const remaining=remainingFor(form.owner,form.sprint); setWarning(remaining!=null&&days>remaining?`${form.owner} will be over capacity by ${days-remaining} day(s) in ${form.sprint}.`: ''); setAllocations(prev=>[{id:id(),featureId:feature.id,featureName:feature.feature_name,workspace:feature.workspace,stage:form.stage,sprint:form.sprint,owner:form.owner,days,isStageComplete:!!form.isStageComplete},...prev]);}
  function saveRecommendedAllocation(feature){const stage=quickPlan.stage||entityNext(feature); if(stage==='Planning Complete')return; const sprintToUse=quickPlan.sprint==='ALL'?sprintOptions[0]:quickPlan.sprint; const owner=quickPlan.owner||ownerOptions[0]||''; const days=Number(quickPlan.days||0); const remaining=remainingFor(owner,sprintToUse); setWarning(remaining!=null&&days>remaining?`${owner} will be over capacity by ${days-remaining} day(s) in ${sprintToUse}.`: ''); const base={id:id(),featureName:feature.feature_name,workspace:feature.workspace,stage,sprint:sprintToUse,owner,days,isStageComplete:!!quickPlan.isStageComplete}; setAllocations(prev=>[feature.planningKey?{...base,planningGroup:'',workspaceName:feature.planningKey}:{...base,featureId:feature.id},...prev]);}
  async function importPlanningExcel(e){const file=e.target.files?.[0]; if(!file)return; try{const XLSX=await import(/* @vite-ignore */ 'https://cdn.sheetjs.com/xlsx-0.20.3/package/xlsx.mjs'); const wb=XLSX.read(await file.arrayBuffer(),{type:'array'}); const sheet=wb.Sheets['26Q1 Planner']; if(!sheet){setImportMessage('Sheet "26Q1 Planner" was not found.');return;} const cell=(r,c)=>sheet[XLSX.utils.encode_cell({r,c})]?.v; const text=(r,c)=>String(cell(r,c)??'').trim(); const sprintCols=Array.from({length:7},(_,i)=>({label:`S${i+1}`,col:17+i*3})); const imported=[]; let currentFeature=''; for(let r=12;r<=175;r++){const featureCell=text(r,2); if(featureCell) currentFeature=featureCell; if(!currentFeature) continue; if(/^name$/i.test(text(r,17))||/^total$/i.test(text(r,17))||/^capacity$/i.test(text(r,18))) continue; sprintCols.forEach(({label,col})=>{const owner=text(r,col); const stage=text(r,col+1); const days=Number(cell(r,col+2)||0); if(owner&&!/^total$/i.test(owner)&&days&&!isRolePlaceholder(owner)){const split=isSplitPlanningGroup(currentFeature); const match=!split?features.find(f=>f.feature_name.trim().toLowerCase()===currentFeature.toLowerCase()):null; imported.push({id:id(),featureId:match?.id||'',featureName:split?'':currentFeature,actualFeatureName:match?.feature_name||'',planningGroup:split?currentFeature:'',workspaceName:currentFeature,workspace:match?.workspace||(!split?currentFeature:''),sprint:`26Q1${label}`,owner,stage:stage||'Build',days,isStageComplete:false});}});} const importedCaps=[]; for(let r=188;r<=198;r++){sprintCols.forEach(({label,col})=>{const owner=text(r,col); const availableDays=Number(cell(r,col+1)||0); if(owner&&!/^total$/i.test(owner)&&Number.isFinite(availableDays)&&availableDays>0&&!isRolePlaceholder(owner)){importedCaps.push({sprint:`26Q1${label}`,owner,availableDays});}});} console.log('imported allocations', imported); console.log('imported capacities', importedCaps); setAllocations(imported); setCapacities(importedCaps); setImportMessage(`Imported ${imported.length} allocation(s). Imported ${importedCaps.length} capacity row(s).`);}catch(err){setImportMessage(`Excel import failed: ${err.message}`);} finally {e.target.value='';}}
  const filteredFeatures=planWorkspaceFilter==='ALL'?features:features.filter(f=>f.workspace===planWorkspaceFilter);
  const workspacePlanSummary=Array.from(new Set([...features.map(f=>f.workspace||'Unknown'),...allocations.filter(a=>!a.featureId&&!isSplitPlanningGroup(a.planningGroup||a.featureName)).map(a=>a.workspace||a.planningGroup||a.featureName||'Unknown')])).sort().map(workspace=>{const items=features.filter(f=>(f.workspace||'Unknown')===workspace); const standalone=allocations.filter(a=>!a.featureId&&!isSplitPlanningGroup(a.planningGroup||a.featureName)&&(a.workspace||a.planningGroup||a.featureName||'Unknown')===workspace); const planned=items.filter(f=>allocations.some(a=>a.featureId===f.id)); const stageCount=stage=>items.filter(f=>allocations.some(a=>a.featureId===f.id&&a.stage===stage)).length+new Set(standalone.filter(a=>a.stage===stage).map(a=>a.featureName||a.planningGroup)).size; const complete=items.filter(f=>PLAN_STAGES.every(stage=>allocations.some(a=>a.featureId===f.id&&a.stage===stage&&a.isStageComplete))).length; return{workspace,total:items.length+new Set(standalone.map(a=>a.featureName||a.planningGroup)).size,planned:planned.length+new Set(standalone.map(a=>a.featureName||a.planningGroup)).size,notPlanned:items.length-planned.length,build:stageCount('Build'),sit:stageCount('SIT'),deploy:stageCount('Deploy'),ba:stageCount('BA Sign Off'),uat:stageCount('UAT Internal'),complete};});
  const queueItems=filteredFeatures.map(f=>{const through=entityThrough(f); const next=entityNext(f); const nextAllocation=allocations.find(a=>a.featureId===f.id&&a.stage===next); return{feature:f,through,next,nextAllocation};}).filter(x=>x.next!=='Planning Complete'&&(ownerFilter==='ALL'||allocations.some(a=>a.featureId===x.feature.id&&a.owner===ownerFilter))&&(sprint==='ALL'||!allocations.some(a=>a.featureId===x.feature.id)||allocations.some(a=>a.featureId===x.feature.id&&a.sprint===sprint)));
  const queueGroups=Array.from(new Set(queueItems.map(x=>x.feature.workspace||'Unknown'))).sort().map(workspace=>{const items=queueItems.filter(x=>(x.feature.workspace||'Unknown')===workspace); const counts=PLAN_STAGES.reduce((m,stage)=>({...m,[stage]:items.filter(x=>x.next===stage).length}),{}); return{workspace,items,total:features.filter(f=>(f.workspace||'Unknown')===workspace).length,notPlanned:items.filter(x=>x.through==='Not planned').length,counts};});
  const kanbanBase=planningFeatures.filter(f=>(planWorkspaceFilter==='ALL'||f.workspace===planWorkspaceFilter)&&(ownerFilter==='ALL'||splitOwners(f.owner).includes(ownerFilter)||entityAllocations(f).some(a=>a.owner===ownerFilter))&&(kanbanSprint==='ALL'||entityAllocations(f).some(a=>a.sprint===kanbanSprint)));
  const kanbanItems=kanbanBase.map(feature=>{const through=entityThrough(feature); const next=entityNext(feature); const sprintAllocations=kanbanSprint==='ALL'?entityAllocations(feature):entityAllocations(feature).filter(a=>a.sprint===kanbanSprint); return{feature,through,next,latest:entityLatestSprint(feature),sprintStage:sprintAllocations.map(a=>`${a.stage}${a.isStageComplete?' ✓':''}`).join(', ')||'—'};}).sort((a,b)=>priorityRank(a.feature.user_count)-priorityRank(b.feature.user_count)||Number(b.feature.user_count||0)-Number(a.feature.user_count||0));
  const kanbanLabels={Build:'Needs Build Plan',SIT:'Needs SIT Plan',Deploy:'Needs Deploy Plan','BA Sign Off':'Needs BA Sign Off Plan','UAT Internal':'Needs UAT Plan','Planning Complete':'Planning Complete'};
  const kanbanColumns=['Build','SIT','Deploy','BA Sign Off','UAT Internal','Planning Complete'].map(stage=>({stage,label:kanbanLabels[stage],items:kanbanItems.filter(item=>item.next===stage)}));
  const selectedAllocations=selectedPlanFeature?entityAllocations(selectedPlanFeature).sort((a,b)=>String(a.sprint).localeCompare(String(b.sprint))||PLAN_STAGES.indexOf(a.stage)-PLAN_STAGES.indexOf(b.stage)):[];
  ...
  const openPlanFeature=feature=>{const stage=entityNext(feature);setSelectedPlanFeature(feature);setQuickPlan(q=>({...q,stage:stage==='Planning Complete'?'UAT Internal':stage,sprint:q.sprint==='ALL'?sprintOptions[0]:q.sprint,owner:q.owner||ownerOptions[0]||'',days:q.days||1,isStageComplete:false}));};
  const kanbanPanel=<div className="panel planning-kanban-panel"><div className="panel-top"><div><h3>Planning Kanban</h3><p className="muted">Start here: highest-priority cards show the next planning decision from actual status and completed allocations.</p></div><div className="toolbar-left"><select value={kanbanSprint} onChange={e=>setKanbanSprint(e.target.value)}><option value="ALL">All Sprints</option>{sprintOptions.map(s=><option key={s}>{s}</option>)}</select><select value={ownerFilter} onChange={e=>setOwnerFilter(e.target.value)}><option value="ALL">All Owners</option>{ownerOptions.map(o=><option key={o}>{o}</option>)}</select><select value={planWorkspaceFilter} onChange={e=>setPlanWorkspaceFilter(e.target.value)}><option value="ALL">All Workspaces</option>{allFeatureWorkspaces.map(w=><option key={w}>{w}</option>)}</select></div></div><div className="board-scroll"><div className="board compact planning-board">{kanbanColumns.map(col=><div className="board-col compact" key={col.stage}><div className="col-head"><div><b>{col.label}</b><small>{col.items.length} shown</small></div><span>{col.items.length}</span></div>{col.items.length?col.items.map(({feature,through,next,latest,sprintStage})=><div className={`feature-card compact ${priorityClass(feature.user_count)}`} key={feature.id} onClick={()=>openPlanFeature(feature)}><div className="card-top"><div className="card-title">{feature.feature_name}</div><button onClick={e=>{e.stopPropagation();openPlanFeature(feature);}}>Edit</button></div><div className="card-workspace">{feature.workspace}</div><div className="card-meta"><span>Current: {STATUS_LABEL[feature.status]||'Standalone'}</span><span>{Number(feature.user_count||0).toLocaleString()} users · {priority(feature.user_count)}</span></div><div className="card-meta"><span>Through: {through}</span><span>Next: {next}</span></div><div className="card-meta"><span>Latest: {latest}</span><span>{kanbanSprint==='ALL'?'':`Sprint: ${sprintStage}`}</span></div></div>):<div className="empty">Nothing here yet.</div>}</div>)}</div></div>{selectedPlanFeature&&<div className="detail-section"><div className="panel-top"><div><h3>{selectedPlanFeature.feature_name}</h3><small>{selectedPlanFeature.workspace} · Current: {STATUS_LABEL[selectedPlanFeature.status]||'Standalone planning item'}</small><small>Planned Through: {entityThrough(selectedPlanFeature)} · Next recommended: {entityNext(selectedPlanFeature)}</small></div><button onClick={()=>setSelectedPlanFeature(null)}>Close</button></div><table><thead><tr><th>Sprint</th><th>Stage</th><th>Owner</th><th>Days</th><th>Outcome</th></tr></thead><tbody>{selectedAllocations.length?selectedAllocations.map(a=><tr key={a.id}><td>{a.sprint}</td><td>{a.stage}</td><td>{a.owner}</td><td>{a.days}</td><td>{a.isStageComplete?'Complete':'Planned only'}</td></tr>):<tr><td colSpan="5">No planning allocations yet.</td></tr>}</tbody></table>{entityNext(selectedPlanFeature)!=='Planning Complete'&&<div className="form-grid"><label>Stage<select value={quickPlan.stage} onChange={e=>setQuickPlan({...quickPlan,stage:e.target.value})}>{PLAN_STAGES.map(s=><option key={s}>{s}</option>)}</select></label><label>Sprint<select value={quickPlan.sprint} onChange={e=>setQuickPlan({...quickPlan,sprint:e.target.value})}>{sprintOptions.map(s=><option key={s}>{s}</option>)}</select></label><label>Owner<select value={quickPlan.owner} onChange={e=>setQuickPlan({...quickPlan,owner:e.target.value})}>{ownerOptions.map(o=><option key={o}>{o}</option>)}</select></label><label>Days<input type="number" min="0" value={quickPlan.days} onChange={e=>setQuickPlan({...quickPlan,days:e.target.value})}/></label><label>Stage outcome<select value={quickPlan.isStageComplete?'complete':'planned'} onChange={e=>setQuickPlan({...quickPlan,isStageComplete:e.target.value==='complete'})}><option value="planned">Planned only</option><option value="complete">Planned and expected to complete</option></select></label><div><small>{capacityGuidance.length?capacityGuidance.map(r=>`${r.owner}: ${r.remaining} days`).join(' · '):'No person-level capacity for this sprint.'}</small>{quickPlan.owner&&selectedRemaining!=null&&<small>{quickPlan.owner} has {selectedRemaining} days remaining in {quickPlan.sprint}.</small>}{selectedRemaining!=null&&Number(quickPlan.days||0)>selectedRemaining&&<p className="capacity-warning">Warning: this exceeds remaining capacity by {Number(quickPlan.days||0)-selectedRemaining} day(s).</p>}</div><div className="modal-actions"><button onClick={()=>saveRecommendedAllocation(selectedPlanFeature)}>Add planning allocation</button></div></div>}</div>}</div>;
  return <div className="dashboard"><div className="dash-head"><div><div className="eyebrow">Delivery Plan</div><h1>Stage-by-stage sprint planning</h1></div><div className="toolbar-left"><button onClick={()=>document.getElementById('planning-xlsx').click()}>Import 26Q1 Planning Excel</button><input id="planning-xlsx" type="file" accept=".xlsx,.xls" hidden onChange={importPlanningExcel}/><select value={sprint} onChange={e=>setSprint(e.target.value)}>{sprintOptions.map(s=><option key={s}>{s}</option>)}</select><select value={ownerFilter} onChange={e=>setOwnerFilter(e.target.value)}><option value="ALL">All Owners</option>{ownerOptions.map(o=><option key={o}>{o}</option>)}</select><select value={planWorkspaceFilter} onChange={e=>setPlanWorkspaceFilter(e.target.value)}><option value="ALL">All Workspaces</option>{allFeatureWorkspaces.map(w=><option key={w}>{w}</option>)}</select></div></div>{importMessage&&<div className="panel import-message">{importMessage}</div>}{kanbanPanel}<div className="two-col"><div className="panel"><h3>Sprint Planner</h3><table><thead><tr><th>Owner</th><th>Available Days</th><th>Planned Days</th><th>Remaining Days</th></tr></thead><tbody>{capacityRows.map(r=><tr key={r.owner}><td>{r.owner}</td><td><input type="number" value={r.available??''} placeholder="—" onChange={e=>updateCapacity(r.owner,e.target.value)}/></td><td>{r.planned}</td><td className={r.remaining<0?'capacity-negative':''}>{r.remaining==null?'—':r.remaining}</td></tr>)}</tbody></table></div><div className="panel"><h3>Add Allocation</h3><div className="form-grid"><label>Feature<select value={form.featureId} onChange={e=>setForm({...form,featureId:e.target.value})}>{features.map(f=><option key={f.id} value={f.id}>{f.feature_name}</option>)}</select></label><label>Stage<select value={form.stage} onChange={e=>setForm({...form,stage:e.target.value})}>{PLAN_STAGES.map(s=><option key={s}>{s}</option>)}</select></label><label>Sprint<select value={form.sprint} onChange={e=>setForm({...form,sprint:e.target.value})}>{sprintOptions.map(s=><option key={s}>{s}</option>)}</select></label><label>Owner<select value={form.owner} onChange={e=>setForm({...form,owner:e.target.value})}>{ownerOptions.map(o=><option key={o}>{o}</option>)}</select></label><label>Days<input type="number" min="0" value={form.days} onChange={e=>setForm({...form,days:e.target.value})}/></label><label>Stage outcome<select value={form.isStageComplete?'complete':'planned'} onChange={e=>setForm({...form,isStageComplete:e.target.value==='complete'})}><option value="planned">Planned only</option><option value="complete">Planned and expected to complete</option></select></label></div>{warning&&<p className="capacity-warning">{warning}</p>}<div className="modal-actions"><button onClick={saveAllocation}>Save</button></div></div></div><div className="panel"><h3>Workspace Planning Summary</h3><table><thead><tr><th>Workspace</th><th>Total</th><th>Planned</th><th>Not planned</th><th>Build</th><th>SIT</th><th>Deploy</th><th>BA Sign Off</th><th>UAT Internal</th><th>Complete</th></tr></thead><tbody>{workspacePlanSummary.map(w=><tr key={w.workspace}><td>{w.workspace}</td><td>{w.total}</td><td>{w.planned}</td><td>{w.notPlanned}</td><td>{w.build}</td><td>{w.sit}</td><td>{w.deploy}</td><td>{w.ba}</td><td>{w.uat}</td><td>{w.complete}</td></tr>)}</tbody></table></div><div className="panel"><h3>Planning Queue</h3>{queueGroups.map(group=><div className="detail-section" key={group.workspace}><div className="panel-top"><div><b>{group.workspace}</b><small>{group.total} total features · {group.notPlanned} not planned</small><small>{PLAN_STAGES.map(stage=>`${stage}: ${group.counts[stage]||0}`).join(' · ')}</small></div></div><div className="queue-grid delivery-queue">{group.items.map(({feature,through,next,nextAllocation})=><div className={`queue-card ${priorityClass(feature.user_count)}`} key={feature.id}><div><b>{feature.feature_name}</b><small>{STATUS_LABEL[feature.status]}</small><small>Planned Through: {through}</small><small>Next to Plan: {next}</small>{nextAllocation&&<small>Next allocation: {nextAllocation.owner} · {nextAllocation.sprint}</small>}</div><span className="user-pill">{feature.user_count||0} users</span></div>)}</div></div>)}</div><div className="two-col"><div className="panel"><h3>Planning Allocations by Sprint</h3>{allocationSprints.length?allocationSprints.map(sp=><div className="detail-section" key={sp}><h4>{sp}</h4>{visibleAllocations.filter(a=>a.sprint===sp).map(a=><div className="queue-card" key={a.id}><div><b>{a.planningGroup || a.featureName || a.workspaceName || a.workspace || 'No planning group'}</b>{a.planningGroup&&a.featureName&&<small>Allocated Feature: {a.featureName}</small>}<small>{a.stage} · {a.owner}</small>{isSplitPlanningGroup(a.planningGroup||a.featureName)&&<label>Allocated Feature<select value={a.featureId||''} onChange={e=>updateAllocationFeature(a.id,e.target.value)}><option value="">Select child feature</option>{getChildFeatureOptions(a,features).map(f=><option key={f.id} value={f.id}>{f.feature_name}</option>)}</select></label>}</div><div className="detail-card-actions"><input type="number" min="0" value={a.days} onChange={e=>updateAllocationDays(a.id,e.target.value)}/><select value={a.isStageComplete?'complete':'planned'} onChange={e=>updateAllocationComplete(a.id,e.target.value==='complete')}><option value="planned">Planned only</option><option value="complete">Complete</option></select><span>{a.sprint}</span></div></div>)}</div>):<p className="muted">No planning allocations yet.</p>}</div><div className="panel"><h3>Planned Days by Owner / Sprint</h3><table><thead><tr><th>Sprint</th><th>Owner</th><th>Planned Days</th><th>Remaining</th></tr></thead><tbody>{Object.values(ownerSprintTotals).map(r=>{const cap=capacities.find(c=>c.sprint===r.sprint&&c.owner===r.owner);return <tr key={`${r.sprint}-${r.owner}`}><td>{r.sprint}</td><td>{r.owner}</td><td>{r.planned}</td><td>{cap?Number(cap.availableDays||0)-r.planned:'—'}</td></tr>})}</tbody></table></div></div></div>;
}
...
function App(){
 ...
 useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null'); if(saved?.features){setFeatures(saved.features);setMilestones(buildWorkspaceMilestones(saved.features,saved.milestones||{}));setAllocations(saved.allocations||[]);setCapacities(saved.capacities||[]);}}catch{} finally { setLoaded(true); }},[]);
 useEffect(()=>{if (loaded) localStorage.setItem(STORAGE_KEY,JSON.stringify({features,milestones,allocations,capacities}));},[features,milestones,allocations,capacities,loaded]);

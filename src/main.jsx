...
function isSplitPlanningGroup(name) {
  return /(pbi|power\s*bi).*dashboard.*reporting|dashboard.*reporting|migration|workspace|parent/i.test(String(name || ''));
}
function planningKeywords(name) {
  return String(name || '').toLowerCase().replace(/power\s*bi|pbi|pba|dashboard|reporting|[-&]/g, ' ').replace(/[^a-z0-9]+/g, ' ').split(/\s+/).filter(w => w && !['and','the','for','of'].includes(w));
}
...
function findFeatureFromComment(comment, parentLabel, features) {
  ...
  const contained=pool.filter(f=>{const key=matchKey(f.feature_name);return key&&commentKey.includes(key);});
  if(contained.length)return{match:contained.length===1?contained[0]:null,matches:contained};
...
function DeliveryPlan({ features, allocations, setAllocations, capacities, setCapacities, customSprints, setCustomSprints, owners, daysOff, setDaysOff, sprintDates, setSprintDates }) {
  ...
  const allFeatureWorkspaces=useMemo(()=>Array.from(new Set(features.map(f=>f.workspace).filter(Boolean))).sort(),[features]);
  ...
  const allocationsByFeatureId=useMemo(()=>{const map=new Map();allocations.forEach(a=>allocationFeatureIds(a).forEach(featureId=>{if(!map.has(featureId))map.set(featureId,[]);map.get(featureId).push(a);}));return map;},[allocations]);
  ...
  const planningFeatures=useMemo(()=>[...features,...standaloneKeys.map(key=>({id:`standalone:${key}`,planningKey:key,feature_name:key,workspace:standaloneAllocationsByKey.get(key)?.[0]?.workspace||key,status:'initial',owner:'',user_count:0}))],[features,standaloneKeys.join('|'),standaloneAllocationsByKey]);
  ...
  const daysOffByOwnerSprint=useMemo(()=>{const map=new Map();daysOff.forEach(row=>{const owner=normalisePersonName(row.owner);const sprintId=normaliseSprintName(row.sprint)||sprintForDateRange(row.startDate,row.endDate,sprintDates);const days=Number(row.days||workingDaysBetween(row.startDate,row.endDate)||0);if(owner&&sprintId&&days>0){const key=ownerSprintKey(owner,sprintId);map.set(key,(map.get(key)||0)+days);}});return map;},[daysOff,sprintDates]);
  ...
  function addDayOff(){const owner=normalisePersonName(dayOffForm.owner);const startDate=dayOffForm.startDate;const endDate=dayOffForm.endDate||startDate;const sprintId=sprintForDateRange(startDate,endDate,sprintDates);const days=workingDaysBetween(startDate,endDate);if(!owner||!startDate||!endDate||!days)return;setDaysOff(prev=>[{id:id(),owner,startDate,endDate,sprint:sprintId,days,note:dayOffForm.note||''},...prev]);setDayOffForm({owner:'',startDate:'',endDate:'',note:''});}
  ...
  function updateAllocationFeature(allocationId,featureId){const feature=features.find(f=>f.id===featureId); if(!feature)return; updateAllocation(allocationId,{featureId:feature.id,featureIds:[feature.id],actualFeatureName:feature.feature_name,featureName:feature.feature_name,workspace:feature.workspace});}
  function updateDiagnosticFeatures(diagnosticIndex,allocationId,featureIds){const cleanIds=Array.from(new Set(featureIds.filter(Boolean)));const selected=cleanIds.map(featureId=>featureById.get(featureId)).filter(Boolean);setAllocations(prev=>prev.map(a=>a.id===allocationId?selected.length?{...a,featureId:selected[0].id,featureIds:selected.map(f=>f.id),actualFeatureName:selected.map(f=>f.feature_name).join(' / '),featureName:selected.map(f=>f.feature_name).join(' / '),workspace:selected[0].workspace,mappingStatus:'matched'}:{...a,featureId:'',featureIds:[],actualFeatureName:'',featureName:a.planningGroup?'':a.featureName,workspace:a.planningGroup?'':a.workspace,mappingStatus:'manual'}:a));setImportDiagnostics(prev=>prev.map((row,idx)=>idx===diagnosticIndex?{...row,matchedFeature:selected.map(f=>f.feature_name).join(' | '),mappingStatus:selected.length?'matched':'manual'}:row));}
    ...
    try{
      ...
      for(let r=12;r<=range.e.r;r++){
        ...
        sprintCols.forEach(({sprint,col})=>{
          ...
          if(owner&&!/^total$/i.test(owner)&&days&&!isRolePlaceholder(owner)){
            ...
            const commentMatch=split&&comment?findFeatureFromComment(comment,currentFeature,features):{match:null,matches:[]};
            const match=commentMatch.match||exactMatch||null;
            const possibleMatches=commentMatch.matches.map(f=>f.feature_name);
            const mappingStatus=comment&&!match&&possibleMatches.length>1?'ambiguous':comment&&!match?'unmatched':match?'matched':'manual';
            const allocationId=id();
            diagnostics.push({allocationId,row:r+1,stageCell:XLSX.utils.encode_cell({r,c:col+1}),sprint,person:owner,feature:currentFeature,stage:sourceStage||stage,days,fillColor:allocationFillColors,expectedComplete:isStageComplete,comment,matchedFeature:match?.feature_name||'',possibleMatches:possibleMatches.join(' | '),mappingStatus});
            imported.push({id:allocationId,featureId:match?.id||'',featureIds:match?[match.id]:[],featureName:match?.feature_name||(split?'':currentFeature),actualFeatureName:match?.feature_name||'',planningGroup:split?currentFeature:'',workspaceName:currentFeature,workspace:match?.workspace||(!split?currentFeature:''),sprint,owner,stage,sourceStage,stageComment:comment,possibleMatches,mappingStatus,days,isStageComplete});
          }
        });
      }
      ...
      for(let r=5;r<=8;r++){
        ...
        if(days>0&&comment){
          ...
          matchedOwners.forEach(owner=>importedDaysOff.push({id:id(),owner,sprint:'26Q1S1',days,startDate:'',endDate:'',note:comment,source:'26Q1 Planner T6:T9'}));
        }
      }
  ...
  const visibleDaysOff=daysOff.filter(row=>sprint==='ALL'||normaliseSprintName(row.sprint||sprintForDateRange(row.startDate,row.endDate,sprintDates))===normaliseSprintName(sprint)).slice().sort((a,b)=>normalisePersonName(a.owner).localeCompare(normalisePersonName(b.owner))||String(a.startDate||'').localeCompare(String(b.startDate||'')));
  ...
  const allocationMatrixRows=Object.values(filteredAllocations.reduce((groups,a)=>{const item=a.planningGroup||a.featureName||a.workspaceName||'Planning item';const key=`${item}||${allocationWorkspace(a)}`;groups[key]||(groups[key]={key,item,workspace:allocationWorkspace(a),allocations:[],days:0});groups[key].allocations.push(a);groups[key].days+=Number(a.days||0);return groups;},{})).sort((a,b)=>a.item.localeCompare(b.item)||a.workspace.localeCompare(b.workspace));
  ...
  const diagnosticOptionCache=useMemo(()=>{const cache=new Map();diagnosticsWithIndex.forEach(row=>{const key=`${row.feature}||${row.possibleMatches||''}`;if(!cache.has(key)){cache.set(key,getChildFeatureOptions({planningGroup:row.feature,featureName:row.feature,workspaceName:row.feature,possibleMatches:row.possibleMatches?row.possibleMatches.split(' | '):[]},features).slice().sort((a,b)=>a.feature_name.localeCompare(b.feature_name)));}});return cache;},[diagnosticsWithIndex,features]);
...
function SprintReview({ allocations, customSprints }) {
  ...
  const rows=useMemo(()=>allocations.filter(a=>sprint==='ALL'||normaliseSprintName(a.sprint)===sprint).map(a=>{const stageText=a.sourceStage||a.stage||'';return{sprint:a.sprint||'Unscheduled',feature:a.actualFeatureName||a.featureName||a.planningGroup||a.workspaceName||'Planning item',stage:stageText,keyGoal:/goal/i.test(stageText),achieved:!!a.isStageComplete};}).sort((a,b)=>String(a.sprint).localeCompare(String(b.sprint))||a.feature.localeCompare(b.feature)||a.stage.localeCompare(b.stage)),[allocations,sprint]);
...
function App(){
 const [features,setFeatures]=useState([]); const [milestones,setMilestones]=useState({}); const [allocations,setAllocations]=useState([]); const [capacities,setCapacities]=useState([]); const [daysOff,setDaysOff]=useState([]); const [sprintDates,setSprintDates]=useState([]); const [customSprints,setCustomSprints]=useState([]); const [mode,setMode]=useState('executive'); const [theme,setTheme]=useState(()=>localStorage.getItem('feature-tracker-theme')||'dark'); const [workspaceFilter,setWorkspaceFilter]=useState('ALL'); const [ownerFilter,setOwnerFilter]=useState('ALL'); const [selectedWorkspace,setSelectedWorkspace]=useState(null); const [editing,setEditing]=useState(null); const [loaded,setLoaded]=useState(false);
 useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null'); if(saved?.features){setFeatures(saved.features);setMilestones(buildWorkspaceMilestones(saved.features,saved.milestones||{}));setAllocations(saved.allocations||[]);setCapacities(saved.capacities||[]);setDaysOff(saved.daysOff||[]);setSprintDates(saved.sprintDates||[]);setCustomSprints(saved.customSprints||[]);}}catch{} finally { setLoaded(true); }},[]);
 useEffect(()=>{if(!loaded)return;const handle=setTimeout(()=>localStorage.setItem(STORAGE_KEY,JSON.stringify({features,milestones,allocations,capacities,daysOff,sprintDates,customSprints})),200);return()=>clearTimeout(handle);},[features,milestones,allocations,capacities,daysOff,sprintDates,customSprints,loaded]);

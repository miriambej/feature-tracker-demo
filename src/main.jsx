...
function DeliveryPlan({ features, allocations, setAllocations, capacities, setCapacities, owners }) {
  const sprintOptions = useMemo(() => Array.from(new Set([fmtDateValue(CURRENT_SPRINT_END), fmtDateValue(NEXT_SPRINT_END), ...capacities.map(c=>c.sprint), ...allocations.map(a=>a.sprint)].filter(Boolean))), [capacities, allocations]);
  const [sprint,setSprint]=useState(sprintOptions[0] || fmtDateValue(CURRENT_SPRINT_END));
  ...
  const [form,setForm]=useState({featureId:'',stage:'Build',sprint:sprintOptions[0] || fmtDateValue(CURRENT_SPRINT_END),owner:ownerOptions[0]||'',days:1});
